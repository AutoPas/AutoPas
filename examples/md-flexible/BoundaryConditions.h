 ...
 * */
 template<typename AutoPasTemplate>
 ...
 */
    std::vector<Molecule> convertToEnteringParticles(AutoPasTemplate &autopas,const std::vector<Molecule> &leavingParticles);
 ...
 */
    auto identifyAndSendHaloParticles(AutoPasTemplate &autoPas);
     ...
     * */
    size_t addEnteringParticles(AutoPasTemplate &autoPas, std::vector<Molecule> enteringParticles);

    /**Adds Halo Particles to the AutoPas Object
     * @param autoPas
     * @param haloParticles
     * */
    void addHaloParticles(AutoPasTemplate &autoPas, std::vector<Molecule> haloParticles);

    /**Realizes periodic Boundaries for the simulation
     * By handling halo particles and updating the container
     * */
    void applyPeriodic(AutoPasTemplate &autoPas);
};
    template <typename AutoPasTemplate>
    std::vector<Molecule> BoundaryConditions<AutoPasTemplate>::convertToEnteringParticles(AutoPasTemplate &autopas, const std::vector<Molecule> &leavingParticles) {
        ...
        for (auto &p : enteringParticles) {
            ...
            for (auto dim = 0; dim < 3; dim++) {
                if (pos[dim] < autopas.getBoxMin()[dim]) {
                    ...
                    pos[dim] = std::min(std::nextafter(autopas.getBoxMax()[dim], -1), pos[dim] + (autopas.getBoxMax()[dim] - autopas.getBoxMin()[dim]));
    ...
    template <typename AutopasTemplate>
    auto BoundaryConditions<AutopasTemplate>::identifyAndSendHaloParticles(AutopasTemplate &autoPas) {
...
template <typename AutopasTemplate>
size_t BoundaryConditions<AutopasTemplate>::addEnteringParticles(AutopasTemplate &autoPas, std::vector<Molecule> enteringParticles) {
        size_t numAdded = 0;
        for (auto &p : enteringParticles) {
...
template <typename AutopasTemplate>
void BoundaryConditions<AutopasTemplate>::addHaloParticles(AutopasTemplate &autoPas, std::vector<Molecule> haloParticles) {
...
template <typename AutopasTemplate>
void BoundaryConditions<AutopasTemplate>::applyPeriodic(AutopasTemplate &autoPas) {
