...
void Simulation::run() {
  ...
  while (needsMoreIterations()) {
    ...
    if (_calculateStatistics and _iteration % statsWriteFrequency == 0) {  // TODO: to change and _iteration > strainResizingStartingIteration
...
void Simulation::applyStrainStressResizing(const double SumOfForcesOnXUpperWall, const double pressure,
  ...
  const double delta_x_abs_calculated = std::abs(200 * a_x_wall * delta_T * delta_T);
  double delta_x =
