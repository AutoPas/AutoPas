...
class Sphere : public Object {
 public:
   ...
   */
  Sphere(const std::array<CalcPrecision, 3> &velocity, unsigned long typeId, const std::array<CalcPrecision, 3> &center,
         int radius, CalcPrecision particleSpacing)
   ...
   * @return center
   */
  [[nodiscard]] const std::array<CalcPrecision, 3> &getCenter() const { return _center; }
   ...
   */
  void iteratePositions(const std::function<void(std::array<CalcPrecision, 3>)> &f) const {
    ...
    for (int z = 0; z <= _radius; ++z) {
      for (int y = 0; y <= _radius; ++y) {
        for (int x = 0; x <= _radius; ++x) {
          ...
          const std::array<CalcPrecision, 3> relativePos = {(CalcPrecision)x, (CalcPrecision)y, (CalcPrecision)z};
          // mirror to rest of sphere
          for (int i = -1; i <= 1; i += 2) {
            for (int k = -1; k <= 1; k += 2) {
              for (int l = -1; l <= 1; l += 2) {
                const std::array<CalcPrecision, 3> mirrorMultipliers = {(CalcPrecision)i, (CalcPrecision)k,
                                                                        (CalcPrecision)l};
                // position mirrored, scaled and absolute
                const std::array<CalcPrecision, 3> posVector =
                    _center + ((relativePos * mirrorMultipliers) * _particleSpacing);

                CalcPrecision distFromCentersSquare =
   ...
   */
  [[nodiscard]] std::array<CalcPrecision, 3> getBoxMin() const override {
    return {_center[0] - ((CalcPrecision)_radius) * _particleSpacing,
            _center[1] - ((CalcPrecision)_radius) * _particleSpacing,
            _center[2] - ((CalcPrecision)_radius) * _particleSpacing};
  }
 ...
 private:
   ...
   */
  std::array<CalcPrecision, 3> _center;
