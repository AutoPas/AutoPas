  ...
  autoPasContainer.forEach(
      [&](auto &particle) {
        ...
        if (not particle.addRDistanceCheck(displacement, maxAllowedDistanceMovedSquared) and
          ...
          // If this condition is violated once this is not necessarily an error. Only if the total distance traveled
          // over the whole rebuild frequency is farther than the skin we lose interactions.
