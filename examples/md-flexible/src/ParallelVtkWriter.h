...
class ParallelVtkWriter {
 ...
 private:
   ...
   */
  std::array<int, 6> calculateWholeExtent(const std::unique_ptr<RegularGridDecomposition> &domainDecomposition);
   ...
   */
  void createPvtsFile(const int &currentIteration, const std::unique_ptr<RegularGridDecomposition> &decomposition);
