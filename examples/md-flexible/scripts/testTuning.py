...
for arg in sys.argv[1:]:
    if "help" in arg:
        print(
            "Usage: ./testTuning.py [sim=path/to/simulation/binary] [tune=tuningStrategy] [log=logLevel] [paths/to/yaml/files or/to/directories]")
        exit(0)
...
def testScenario(yamlFile):
    ...
    if not expected:
        ...
        print(
            YELLOW + "Selected configuration faster than expected! Estimates maybe invalid for this hardware!" + ENDCOLOR)
