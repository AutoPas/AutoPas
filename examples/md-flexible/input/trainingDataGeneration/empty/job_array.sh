#!/bin/bash
#SBATCH --job-name=Empty_Job_Array
#SBATCH --nodes=1
...
# The data collection only needs one rank, so this is not strictly nessecary even if running MPI simulations
...
cd ${thread_count[${SLURM_ARRAY_TASK_ID}]}_threads/skin_${verlet_skin[${SLURM_ARRAY_TASK_ID}]}/
...
do
    ...
    mpirun -np 1 $HOME/AutoPas/build-MPI/examples/md-flexible/md-flexible --use-tuning-log true --yaml-filename input.yaml  > logOutput_${SLURM_JOB_ID}_${SLURM_ARRAY_TASK_ID}.out 2>&1
    cd ..
done
