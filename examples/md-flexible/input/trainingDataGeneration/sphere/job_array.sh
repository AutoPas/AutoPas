#!/bin/bash
#SBATCH --job-name=Sphere_Job_Array
#SBATCH --nodes=1
...
cd radius_${radius[${SLURM_ARRAY_TASK_ID}]}/spacing_${spacing[${SLURM_ARRAY_TASK_ID}]}/${thread_count[${SLURM_ARRAY_TASK_ID}]}_threads/skin_${verlet_skin[${SLURM_ARRAY_TASK_ID}]}/
