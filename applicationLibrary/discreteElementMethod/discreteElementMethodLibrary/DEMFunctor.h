...
#pragma omp simd reduction(+ : fxacc, fyacc, fzacc)
      for (unsigned int j = i + 1; j < soa.getNumberOfParticles(); j++) {
        ...
        // Newton3
        fxptr[j] -= _factorSubtractExcessForces * fx;
        fyptr[j] -= _factorSubtractExcessForces * fy;
        fzptr[j] -= _factorSubtractExcessForces * fz;        

      }

    fxptr[i] += _factorSubtractExcessForces * fxacc;
    fyptr[i] += _factorSubtractExcessForces * fyacc;
    fzptr[i] += _factorSubtractExcessForces * fzacc;
...
#pragma omp simd reduction(+ : fxacc, fyacc, fzacc)
      for (unsigned int j = 0; j < soa2.getNumberOfParticles(); ++j) {
        ...
        const SoAFloatPrecision fx = _factorSubtractExcessForces * (drx * normalForce - dvx * normalDamping);
        const SoAFloatPrecision fy = _factorSubtractExcessForces * (dry * normalForce - dvy * normalDamping);
        const SoAFloatPrecision fz = _factorSubtractExcessForces * (drz * normalForce - dvz * normalDamping);
