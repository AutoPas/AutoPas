...
#include "NeighborIdentificationFunctorTest.h"
...
#include "molecularDynamics/molecularDynamicsLibrary/LJMultisiteFunctor.h"
...
using NeighborIdentificationFunc = autopas::NeighborIdentificationFunctor<ParticleType>;
using InteractionListTypeAoS = NeighborIdentificationFunc::NeighborListAoSType;
 ...
 */
void fillCell(CellType &cell, const std::vector<std::array<double, 3>> &positions,
              const std::vector<OwnershipState> &ownerships, const size_t idOffset = 0) {
  for (size_t i = 0; i < positions.size(); ++i) {
    ParticleType m(positions[i], {0., 0., 0.}, i + idOffset);
 ...
 */
void initMap(InteractionListTypeAoS &interactionList, CellType &cell) {
 ...
 */
std::vector<CellType> makeGridCells() {
  ...
  std::vector<CellType> cells(numCells);
  for (size_t cellID = 0; cellID < numCells; ++cellID) {
    const ParticleType defaultParticle({0., 0., 0.}, {0., 0., 0.}, cellID * particlesPerCell);
 ...
 */
std::vector<size_t> neighborIDs(const InteractionListTypeAoS &map, CellType &cell, size_t index) {
 ...
 */
void compareWithExpectedLists(const InteractionListTypeAoS &map, CellType &cell, const ExpectedLists &expected) {
 ...
 */
void expectListsMatch(const InteractionListTypeAoS &map, CellType &cell, const InteractionListTypeAoS &referenceMap,
 ...
 */
void applyAoSOverAllPairs(NeighborIdentificationFunc &functor, CellType &cell, bool n3Used) {
  for (size_t i = 0; i < cell.size(); ++i) {
 ...
 */
TEST_P(NeighborIdentificationFunctorListGenTest, AoSFunctor) {
  ...
  NeighborIdentificationFunc functor(map, cutoffLength, params.n3Mode.gatherN3Lists);
 ...
 */
TEST_F(NeighborIdentificationFunctorTest, AoSSingleCallDirectionality) {
  const auto run = [](bool gatherN3Lists, bool newton3) {
    CellType cell;
    fillCell(cell, {{0., 0., 0.}, {1., 0., 0.}}, std::vector<OwnershipState>(2, OwnershipState::owned));
    InteractionListTypeAoS map;
    initMap(map, cell);
    NeighborIdentificationFunc functor(map, cutoffLength, gatherN3Lists);
 ...
 */
TEST_P(NeighborIdentificationFunctorListGenTest, SoAFunctorSingle) {
  const auto &params = GetParam();
  CellType cell;
  fillCell(cell, params.positions, params.ownerships);
  InteractionListTypeAoS map;
 ...
 */
TEST_P(NeighborIdentificationFunctorSoAPairTest, NeighborLists) {
  const auto &params = GetParam();
  CellType cell1, cell2;
...
INSTANTIATE_TEST_SUITE_P(Generated, NeighborIdentificationFunctorSoAPairTest,
 ...
 */
TEST_P(NeighborIdentificationFunctorListGenTest, SoAFunctorVerlet) {
  const auto &params = GetParam();
  CellType cell;
  fillCell(cell, params.positions, params.ownerships);
  InteractionListTypeAoS map;
 ...
 */
INSTANTIATE_TEST_SUITE_P(Generated, NeighborIdentificationFunctorListGenTest,
 ...
 */
TEST_F(NeighborIdentificationFunctorTest, N3ListsWithoutN3Throws) {
  CellType cell1, cell2;
  ...
  NeighborIdentificationFunc functor(map, cutoffLength, /*gatherNewton3Lists*/ true);
  functor.SoALoader(cell1, cell1._particleSoABuffer, 0, /*skipSoAResize*/ false);
  functor.SoALoader(cell2, cell2._particleSoABuffer, 0, /*skipSoAResize*/ false);
 ...
 */
TEST_F(NeighborIdentificationFunctorTest, AllowsNonNewton3DependsOnGatherN3Lists) {
  InteractionListTypeAoS map;
  NeighborIdentificationFunc noN3ListsFunctor(map, cutoffLength, /*gatherNewton3Lists*/ false);
  NeighborIdentificationFunc n3ListsFunctor(map, cutoffLength, /*gatherNewton3Lists*/ true);
 ...
 */
TEST_P(NeighborIdentificationFunctorThreadSafeTest, AoSFunctor) {
  const auto &mode = GetParam();
  constexpr std::array<size_t, 3> particlesPerDim{2, 2, 1};

  // Serial reference.
  CellType referenceCell;
  autopasTools::generators::GridGenerator::fillWithParticles(referenceCell, particlesPerDim, ParticleType{},
                                                             {1., 1., 1.}, {0., 0., 0.});
  InteractionListTypeAoS referenceMap;
  initMap(referenceMap, referenceCell);
  NeighborIdentificationFunc referenceFunctor(referenceMap, cutoffLength, mode.gatherN3Lists);
  ...
  CellType cell;
  autopasTools::generators::GridGenerator::fillWithParticles(cell, particlesPerDim, ParticleType{}, {1., 1., 1.},
                                                             {0., 0., 0.});
  InteractionListTypeAoS map;
  initMap(map, cell);
  NeighborIdentificationFunc functor(map, cutoffLength, mode.gatherN3Lists);
  {
 ...
 * not thread safe.
 */
TEST_P(NeighborIdentificationFunctorThreadSafeTest, SoAFunctorSingle) {
  const auto &mode = GetParam();
 ...
 */
TEST_P(NeighborIdentificationFunctorThreadSafeTest, SoAFunctorPair) {
  const auto &mode = GetParam();
 ...
 */
TEST_P(NeighborIdentificationFunctorThreadSafeTest, SoAFunctorVerletPairwiseLists) {
  const auto &mode = GetParam();
...
INSTANTIATE_TEST_SUITE_P(Generated, NeighborIdentificationFunctorThreadSafeTest, ::testing::ValuesIn(n3Modes),
                         ParamNameGenerator());
 ...
 * n3 and non-n3 lists.
 */
TEST_F(NeighborIdentificationFunctorTest, ThreadSafeSoAFunctorVerlet) {
  constexpr std::array<size_t, 3> particlesPerDim{2, 2, 1};
  ...
  for (const bool gatherN3Lists : {false, true}) {
    ...
    InteractionListTypeAoS referenceMap;
    initMap(referenceMap, referenceCell);
    NeighborIdentificationFunc referenceFunctor(referenceMap, cutoffLength, gatherN3Lists);
    referenceFunctor.SoALoader(referenceCell, referenceCell._particleSoABuffer, 0, /*skipSoAResize*/ false);
    for (size_t i = 0; i < referenceCell.size(); ++i) {
 ...
 */
TEST_F(NeighborIdentificationFunctorTest, InitializeNeighborListSeedsAndClearsMap) {
  ...
  autoPas.addParticle(ParticleType({1., 1., 1.}, {0., 0., 0.}, 0));
  autoPas.addParticle(ParticleType({2., 1., 1.}, {0., 0., 0.}, 1));
  autoPas.addParticle(ParticleType({3., 1., 1.}, {0., 0., 0.}, 2));

  InteractionListTypeAoS neighborLists;
  NeighborIdentificationFunc functor(neighborLists, cutoffLength, /*gatherNewton3Lists*/ false);
  ...
  ParticleType staleParticle({0., 0., 0.}, {0., 0., 0.}, 99);
 ...
 */
TEST_P(NeighborIdentificationFunctorFullFlowTest, ForcesMatchDirectLJApplication) {
  ...
  CellType cell, referenceCell;
  for (CellType *cellPtr : {&cell, &referenceCell}) {
    autopasTools::generators::GridGenerator::fillWithParticles(*cellPtr, particlesPerDim, ParticleType{}, {1., 1., 1.},
                                                               {0., 0., 0.});
  ...
  InteractionListTypeAoS map;
  initMap(map, cell);
  NeighborIdentificationFunc listGenFunctor(map, cutoffLength, mode.gatherN3Lists);
  applyAoSOverAllPairs(listGenFunctor, cell, mode.n3Used);

  mdLib::LJFunctor<ParticleType> ljFunctor(cutoffLength);
...
INSTANTIATE_TEST_SUITE_P(Generated, NeighborIdentificationFunctorFullFlowTest,
 ...
 */
TEST_F(NeighborIdentificationFunctorTest, ComputeInteractionsInitializesMap) {
  ...
  InteractionListTypeAoS neighborLists;
  NeighborIdentificationFunc functor(neighborLists, /*interactionLength*/ 1.0, /*gatherNewton3Lists*/ false);

  // Two particles added into the container.
  autoPas.addParticle(ParticleType({1.0, 1.0, 1.0}, {0., 0., 0.}, 0));
  autoPas.addParticle(ParticleType({1.5, 1.0, 1.0}, {0., 0., 0.}, 1));
  ...
  {
    std::vector<ParticleType *> particlePtrs;
    for (auto iter = autoPas.begin(); iter.isValid(); ++iter) {
  ...
  autoPas.addParticle(ParticleType({8.0, 8.0, 8.0}, {0., 0., 0.}, 2));
  autoPas.addParticle(ParticleType({8.5, 8.0, 8.0}, {0., 0., 0.}, 3));
  autoPas.addHaloParticle(ParticleType({-0.5, 1.0, 1.0}, {0., 0., 0.}, 4));
  autoPas.addHaloParticle(ParticleType({10.5, 8.0, 8.0}, {0., 0., 0.}, 5));
