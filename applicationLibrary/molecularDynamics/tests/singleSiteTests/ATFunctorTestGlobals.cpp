...
void ATFunctorTestGlobals::ATFunctorTestGlobalsNoMixing(bool newton3) {
  mdLib::AxilrodTellerFunctor<Molecule, false, autopas::FunctorN3Modes::Both, true> functor(cutoff);
...
//  const double expectedVirial = calculateATVirialTotal(p1Pos, p2Pos, p3Pos, cutoff, nu);

  EXPECT_NEAR(potentialEnergy, expectedEnergy, absDelta) << ", newton3: " << newton3;
//  EXPECT_NEAR(virial, whereFactor * expectedVirial, absDelta) << "where: " << where_str << ", newton3: " << newton3;
}

//template <class FuncType>
//void ATFunctorTestGlobals<FuncType>::testAoSGlobalsMixedN3(ATFunctorTestGlobals<FuncType>::where_type where) {
//  double xOffset;
...
//      functor.endTraversal(newton3);
//
//      const auto finalPotentialEnergy = functor.getPotentialEnergy();
...
//        EXPECT_NEAR(finalPotentialEnergy, refPotentialEnergy, absDelta)
...
//            << "), where: " << where_str << ", global newton3: " << newton3;
//      } else {
//        refPotentialEnergy = finalPotentialEnergy;
...
//
//template <class FuncType>
//void ATFunctorTestGlobals<FuncType>::testSoAGlobals(ATFunctorTestGlobals<FuncType>::where_type where, bool newton3,
...
//
//  mdLib::ATFunctor<Molecule, shifting, mixing, autopas::FunctorN3Modes::Both, true> functor(cutoff);
//  functor.setParticleProperties(epsilon * 24, sigma);
//  double xOffset;
...
//  EXPECT_NEAR(potentialEnergy, whereFactor * expectedEnergy * 2., absDelta)
...
TEST_P(ATFunctorTestGlobals, testAoSFunctorGlobals) {
  auto [mixing, newton3] = GetParam();
  if (mixing) {

  }
  else {
    ATFunctorTestGlobalsNoMixing(newton3);
  }

}

//TEST_P(ATFunctorTestGlobals, testAoSFunctorGlobalsMixedN3) {
...
//TEST_P(ATFunctorTestGlobals, testSoAFunctorGlobalsOwn) {
//  using FuncType = TypeParam;
//  using TestType = ATFunctorTestGlobals<FuncType>;
//
...
//TEST_P(ATFunctorTestGlobals, testSoAFunctorGlobalsVerlet) {
//  using FuncType = TypeParam;
//  using TestType = ATFunctorTestGlobals<FuncType>;
//
//  for (bool mixedNewton3FunctorCalls : {false, true}) {
...
//TEST_P(ATFunctorTestGlobals, testSoAFunctorGlobalsPair) {
//  using FuncType = TypeParam;
//  using TestType = ATFunctorTestGlobals<FuncType>;
//
//  for (bool mixedNewton3FunctorCalls : {false, true}) {
...
//TEST_P(ATFunctorTestGlobals, testAoSFunctorGlobalsOpenMPParallel) {
//  using FuncType = TypeParam;
//  using TestType = ATFunctorTestGlobals<FuncType>;
//
...
//TEST_P(ATFunctorTestGlobals, testFunctorGlobalsThrowBad) {
...
static auto toString = [](const auto &info) {
  auto [mixing, newton3] = info.param;
  std::stringstream resStream;
  resStream << (mixing ? "Mixing" : "noMixing") << "_" << (newton3 ? "N3" : "noN3");
...
INSTANTIATE_TEST_SUITE_P(Generated, ATFunctorTestGlobals, ::testing::Combine(::testing::Bool(), ::testing::Bool()), toString);
