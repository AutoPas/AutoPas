...
class LJFunctorMIPP
    : public autopas::PairwiseFunctor<Particle, LJFunctorMIPP<Particle, applyShift, useMixing, useNewton3,
 ...
 private:
  ...
  explicit LJFunctorMIPP(double cutoff, void * /*dummy*/)
      : autopas::PairwiseFunctor<Particle, LJFunctorMIPP<Particle, applyShift, useMixing, useNewton3, calculateGlobals,
 ...
 public:
  ...
  std::string getName() final { return "LJFunctorMIPP"; }
 ...
 private:
  ...
  inline void SoAKernel(const size_t j, const Reg<int64_t> &ownedStateI, const int64_t *const __restrict ownedStatePtr2,
                        const Reg<double> &x1, const Reg<double> &y1, const Reg<double> &z1,
                        ...
                        const size_t *const typeID1ptr, const size_t *const typeID2ptr, Reg<double> &fxacc,
                        Reg<double> &fyacc, Reg<double> &fzacc, Reg<double> *virialSumX, Reg<double> *virialSumY,
    ...
    const Reg<int64_t> ownedStateJ =
        remainderIsMasked ? maskzlds(_masks[rest - 1], &ownedStatePtr2[j]) : loadu(&ownedStatePtr2[j]);
    ...
    const Reg<double> facMasked = remainderIsMasked ? blend(fac, _zero, andb(cutoffDummyMask, _masks[rest - 1]))
    ...
    if constexpr (newton3) {
      const Reg<double> fx2 = remainderIsMasked ? maskzlds(_masks[rest - 1], &fx2ptr[j]) : loadu(&fx2ptr[j]);
      const Reg<double> fy2 = remainderIsMasked ? maskzlds(_masks[rest - 1], &fy2ptr[j]) : loadu(&fy2ptr[j]);
      const Reg<double> fz2 = remainderIsMasked ? maskzlds(_masks[rest - 1], &fz2ptr[j]) : loadu(&fz2ptr[j]);

      const Reg<double> fx2new = sub(fx2, fx);
      const Reg<double> fy2new = sub(fy2, fy);
      const Reg<double> fz2new = sub(fz2, fz);

      remainderIsMasked ? masksts(_masks[rest - 1], &fx2ptr[j], fx2new) : storeu(&fx2ptr[j], fx2new);
      remainderIsMasked ? masksts(_masks[rest - 1], &fy2ptr[j], fy2new) : storeu(&fy2ptr[j], fy2new);
      remainderIsMasked ? masksts(_masks[rest - 1], &fz2ptr[j], fz2new) : storeu(&fz2ptr[j], fz2new);
    }
    ...
    if constexpr (calculateGlobals) {
      ...
      const Reg<double> upotMasked = remainderIsMasked ? blend(upot, _zero, andb(cutoffDummyMask, _masks[rest - 1]))
