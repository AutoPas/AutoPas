...
[[nodiscard]] double cos_2theta(const double& cos_theta) {
  return 2 * Math::pow<2>(cos_theta) -1;
}
...
[[nodiscard]] double cos_3theta(const double& cos_theta) {
  return 4 * Math::pow<3>(cos_theta) - 3 * cos_theta;
}
...
[[nodiscard]] double sin_theta(const double& cos_theta) {
  return std::sqrt(1 - Math::pow<2>(cos_theta));
}
...
[[nodiscard]] double W_111(const DisplacementHandle& displacementIJ, const DisplacementHandle& displacementJK, const DisplacementHandle& displacementKI,
...
[[nodiscard]] double W_112(const DisplacementHandle& displacementIJ, const DisplacementHandle& displacementJK, const DisplacementHandle& displacementKI,
                           const CosineHandle& cosineI, const CosineHandle& cosineJ, const CosineHandle& cosineK) {
  const auto IJ = L2Norm(displacementIJ.getDisplacement());
  const auto JK = L2Norm(displacementJK.getDisplacement());
  const auto KI = L2Norm(displacementKI.getDisplacement());
  ...
  return 3. / 16 / (Math::pow<3>(IJ) * Math::pow<4>(JK) * Math::pow<4>(KI)) * (9 * cosK - 25 * cos3K + 6 * cosIminusJ * (3 + 5 * cos2K));
}

[[nodiscard]] double W_211(const DisplacementHandle& displacementIJ, const DisplacementHandle& displacementJK, const DisplacementHandle& displacementKI,
                           const CosineHandle& cosineI, const CosineHandle& cosineJ, const CosineHandle& cosineK) {
...
[[nodiscard]] double W_121(const DisplacementHandle& displacementIJ, const DisplacementHandle& displacementJK, const DisplacementHandle& displacementKI,
                           const CosineHandle& cosineI, const CosineHandle& cosineJ, const CosineHandle& cosineK) {
  // no need to call .getInv() on the displacements as we only take their L2Norm
...
[[nodiscard]] double W_122(const DisplacementHandle& displacementIJ, const DisplacementHandle& displacementJK, const DisplacementHandle& displacementKI,
                           const CosineHandle& cosineI, const CosineHandle& cosineJ, const CosineHandle& cosineK) {
  const auto IJ = L2Norm(displacementIJ.getDisplacement());
  const auto JK = L2Norm(displacementJK.getDisplacement());
  const auto KI = L2Norm(displacementKI.getDisplacement());
  ...
  return 15. / 64 / (Math::pow<4>(IJ) * Math::pow<5>(JK) * Math::pow<4>(KI)) * (3 * (cosI + 5 * cos3I) + 20 * cosJminusK * (1 - 3 * cos2I) + 70 * cos2JminusK * cosI);

}

[[nodiscard]] double W_212(const DisplacementHandle& displacementIJ, const DisplacementHandle& displacementJK, const DisplacementHandle& displacementKI,
                           const CosineHandle& cosineI, const CosineHandle& cosineJ, const CosineHandle& cosineK) {
  // no need to call .getInv() on the displacements as we only take their L2Norm
...
[[nodiscard]] double W_221(const DisplacementHandle& displacementIJ, const DisplacementHandle& displacementJK, const DisplacementHandle& displacementKI,
                           const CosineHandle& cosineI, const CosineHandle& cosineJ, const CosineHandle& cosineK) {
  // no need to call .getInv() on the displacements as we only take their L2Norm
  // permutation of i and k
...
[[nodiscard]] double W_222(const DisplacementHandle& displacementIJ, const DisplacementHandle& displacementJK, const DisplacementHandle& displacementKI,
                           const CosineHandle& cosineI, const CosineHandle& cosineJ, const CosineHandle& cosineK) {
  const auto IJ = L2Norm(displacementIJ.getDisplacement());
  const auto JK = L2Norm(displacementJK.getDisplacement());
  const auto KI = L2Norm(displacementKI.getDisplacement());
  ...
  return 15. / 128 / (Math::pow<5>(IJ) * Math::pow<5>(JK) * Math::pow<5>(KI)) * ( -27 + 220 * cosI * cosJ * cosK + 490 * cos2I * cos2J * cos2K + 175 * (cos2IminusJ + cos2JminusK + cos2KminusI));

}

[[nodiscard]] double W_113(const DisplacementHandle& displacementIJ, const DisplacementHandle& displacementJK, const DisplacementHandle& displacementKI,
                           const CosineHandle& cosineI, const CosineHandle& cosineJ, const CosineHandle& cosineK) {
  const auto IJ = L2Norm(displacementIJ.getDisplacement());
  const auto JK = L2Norm(displacementJK.getDisplacement());
  const auto KI = L2Norm(displacementKI.getDisplacement());
  ...
  return 5. / 32 / (Math::pow<3>(IJ) * Math::pow<5>(JK) * Math::pow<5>(KI)) * (9 + 8 * cos2K - 49 * cos4K + 6 * cosIminusJ * (9 * cosK + 7 * cos3K));
}

[[nodiscard]] double W_311(const DisplacementHandle& displacementIJ, const DisplacementHandle& displacementJK, const DisplacementHandle& displacementKI,
                           const CosineHandle& cosineI, const CosineHandle& cosineJ, const CosineHandle& cosineK) {
  // no need to call .getInv() on the displacements as we only take their L2Norm
  // permutation of i and k
...
[[nodiscard]] double W_131(const DisplacementHandle& displacementIJ, const DisplacementHandle& displacementJK, const DisplacementHandle& displacementKI,
                           const CosineHandle& cosineI, const CosineHandle& cosineJ, const CosineHandle& cosineK) {
  // no need to call .getInv() on the displacements as we only take their L2Norm
  // permutation of j and k
...
template <size_t ID>
[[nodiscard]] double W_111_deriv_wrt(const DisplacementHandle& displacementIJ, const DisplacementHandle& displacementJK, const DisplacementHandle& displacementKI,
                                     const CosineHandle& cosineI, const CosineHandle& cosineJ, const CosineHandle& cosineK) {
  const auto IJ = L2Norm(displacementIJ.getDisplacement());
  const auto JK = L2Norm(displacementJK.getDisplacement());
  const auto KI = L2Norm(displacementKI.getDisplacement());
  ...
  return 9. / (Math::pow<3>(IJ) * Math::pow<3>(JK) * Math::pow<3>(KI)) * (nablaDisplacementTerm * cosineTerm + nablaCosineTerm);
}

template <size_t ID>
[[nodiscard]] double W_112_deriv_wrt(const DisplacementHandle& displacementIJ, const DisplacementHandle& displacementJK, const DisplacementHandle& displacementKI,
                                     const CosineHandle& cosineI, const CosineHandle& cosineJ, const CosineHandle& cosineK) {
  const auto IJ = L2Norm(displacementIJ.getDisplacement());
  const auto JK = L2Norm(displacementJK.getDisplacement());
  const auto KI = L2Norm(displacementKI.getDisplacement());
  ...
  const auto nablaCosineTerm = 9 * nablaCosK - 25 * nablaCos3K + 6 * nablaCosIminusJ * (3 + 5 * cos2K) + 30 * cosIminusJ * nablaCos2K;

  return 3. / 16 / (Math::pow<3>(IJ) + Math::pow<4>(JK) + Math::pow<4>(KI)) * (nablaDisplacementTerm * cosineTerm + nablaCosineTerm);
}

template <size_t ID>
[[nodiscard]] double W_211_deriv_wrt(const DisplacementHandle& displacementIJ, const DisplacementHandle& displacementJK, const DisplacementHandle& displacementKI,
                                     const CosineHandle& cosineI, const CosineHandle& cosineJ, const CosineHandle& cosineK) {
  // permutation of i and k
  return W_112_deriv_wrt<ID>(displacementJK.getInv(), displacementIJ.getInv(), displacementKI.getInv(), cosineK, cosineJ, cosineI);
}

template <size_t ID>
[[nodiscard]] double W_121_deriv_wrt(const DisplacementHandle& displacementIJ, const DisplacementHandle& displacementJK, const DisplacementHandle& displacementKI,
                                     const CosineHandle& cosineI, const CosineHandle& cosineJ, const CosineHandle& cosineK) {
  // permutation of j and k
  return W_112_deriv_wrt<ID>(displacementKI.getInv(), displacementJK.getInv(), displacementIJ.getInv(), cosineI, cosineK, cosineJ);
}

template <size_t ID>
[[nodiscard]] double W_122_deriv_wrt(const DisplacementHandle& displacementIJ, const DisplacementHandle& displacementJK, const DisplacementHandle& displacementKI,
                                     const CosineHandle& cosineI, const CosineHandle& cosineJ, const CosineHandle& cosineK) {
  const auto IJ = L2Norm(displacementIJ.getDisplacement());
  const auto JK = L2Norm(displacementJK.getDisplacement());
  const auto KI = L2Norm(displacementKI.getDisplacement());
  ...
  const auto nablaCosineTerm = 3 * nablaCosI + 15 * nablaCos3I + 20 * nablaCosJminusK * (1 - 3 * cos2I) - 60 * cosJminusK * nablaCos2I + 70 * nablaCos2JminusK * cosI + 70 * cos2JminusK * nablaCosI;

  return 15. / 64 / (Math::pow<4>(IJ) + Math::pow<5>(JK) + Math::pow<4>(KI)) * (nablaDisplacementTerm * cosineTerm + nablaCosineTerm);
}

template <size_t ID>
[[nodiscard]] double W_212_deriv_wrt(const DisplacementHandle& displacementIJ, const DisplacementHandle& displacementJK, const DisplacementHandle& displacementKI,
                                     const CosineHandle& cosineI, const CosineHandle& cosineJ, const CosineHandle& cosineK) {
  // permutation of i and j
  return W_122_deriv_wrt<ID>(displacementIJ.getInv(), displacementKI.getInv(), displacementJK.getInv(), cosineJ, cosineI, cosineK);
}

template <size_t ID>
[[nodiscard]] double W_221_deriv_wrt(const DisplacementHandle& displacementIJ, const DisplacementHandle& displacementJK, const DisplacementHandle& displacementKI,
                                     const CosineHandle& cosineI, const CosineHandle& cosineJ, const CosineHandle& cosineK) {
  // permutation of i and k
  return W_122_deriv_wrt<ID>(displacementJK.getInv(), displacementIJ.getInv(), displacementKI.getInv(), cosineK, cosineJ, cosineI);
}

template <size_t ID>
[[nodiscard]] double W_222_deriv_wrt(const DisplacementHandle& displacementIJ, const DisplacementHandle& displacementJK, const DisplacementHandle& displacementKI,
                                     const CosineHandle& cosineI, const CosineHandle& cosineJ, const CosineHandle& cosineK) {
  const auto IJ = L2Norm(displacementIJ.getDisplacement());
  const auto JK = L2Norm(displacementJK.getDisplacement());
  const auto KI = L2Norm(displacementKI.getDisplacement());
  ...
  const auto cosineTerm = - 27 + 220 * cosI * cosJ * cosK + 490 * cos2I * cos2J * cos2K + 175 * (cos2IminusJ + cos2JminusK + cos2KminusI);
  const auto nablaCosineTerm1 = 220 * (nablaCosI * cosJ * cosK + cosI * nablaCosJ * cosK + cosI * cosJ * nablaCosK);
  const auto nablaCosineTerm2 = 490 * (nablaCos2I * cos2J * cos2K + cos2I * nablaCos2J * cos2K + cos2I * cos2J * nablaCos2K);
  const auto nablaCosineTerm3 = 175 * (nablaCos2IminusJ + nablaCos2JminusK + nablaCos2KminusI);

  return 15. / 128 / (Math::pow<5>(IJ) + Math::pow<5>(JK) + Math::pow<5>(KI)) * (nablaDisplacementTerm * cosineTerm + nablaCosineTerm1 + nablaCosineTerm2 + nablaCosineTerm3);

}

template <size_t ID>
[[nodiscard]] double W_113_deriv_wrt(const DisplacementHandle& displacementIJ, const DisplacementHandle& displacementJK, const DisplacementHandle& displacementKI,
                                     const CosineHandle& cosineI, const CosineHandle& cosineJ, const CosineHandle& cosineK) {
  const auto IJ = L2Norm(displacementIJ.getDisplacement());
  const auto JK = L2Norm(displacementJK.getDisplacement());
  const auto KI = L2Norm(displacementKI.getDisplacement());
  ...
  const auto nablaCosineTerm = 8 * nablaCos2K - 49 * nablaCos4K + 6 * nablaCosIminusJ * (9 * cosK + 7 * cos3K) + 6 * cosIminusJ * (9 * nablaCosK + 7 * nablaCos3K);

  return 5. / 32 / (Math::pow<3>(IJ) + Math::pow<5>(JK) + Math::pow<5>(KI)) * (nablaDisplacementTerm * cosineTerm + nablaCosineTerm);

}

template <size_t ID>
[[nodiscard]] double W_131_deriv_wrt(const DisplacementHandle& displacementIJ, const DisplacementHandle& displacementJK, const DisplacementHandle& displacementKI,
                                     const CosineHandle& cosineI, const CosineHandle& cosineJ, const CosineHandle& cosineK) {
  // permuation of j and k
  return W_113_deriv_wrt<ID>(displacementKI.getInv(), displacementJK.getInv(), displacementIJ.getInv(), cosineI, cosineK, cosineJ);
}

template <size_t ID>
[[nodiscard]] double W_311_deriv_wrt(const DisplacementHandle& displacementIJ, const DisplacementHandle& displacementJK, const DisplacementHandle& displacementKI,
                                     const CosineHandle& cosineI, const CosineHandle& cosineJ, const CosineHandle& cosineK) {
  // permutation of i and k
  return W_113_deriv_wrt<ID>(displacementJK.getInv(), displacementIJ.getInv(), displacementKI.getInv(), cosineK, cosineJ, cosineI);
}
