/**
 * @file LJMultisiteFunctorAVX_STS.h
 * @date 11/09/2023
 * @author S. Newcome
 */
#pragma once
 ...
 *
 * The cutoff criterion for site-to-site pairs are solely based on site-to-site distances.
 *
    ...
    for (size_t molA = 0; molA < soaA.getNumberOfParticles(); ++molA) {
      ...
      for (size_t siteA = 0; siteA < noSitesInMolA; ++siteA) {
        ...
        SoAKernel<newton3, true>(
            siteTypesB, exactSitePositionBx, exactSitePositionBy, exactSitePositionBz, siteForceBx,
...
#pragma message "SoAFunctorVerlet functor called without AVX support!"
  ...
  inline void SoAKernel(const std::vector<size_t, autopas::AlignedAllocator<size_t>> &siteTypesB,
    ...
    for (size_t siteVectorIndex = 0; siteVectorIndex < siteTypesB.size(); siteVectorIndex += vecLength) {
      ...
      if constexpr (useMixing) {
        ...
        __m256i siteTypeIndices = autopas::utils::avx::load_epi64(remainderCase, &siteTypesB[siteVectorIndex + offset], remainderMask);
      ...
      const __m256d cutoffCondition = _mm256_cmp_pd(distanceSquared, _cutoffSquared, _CMP_LE_OQ);
