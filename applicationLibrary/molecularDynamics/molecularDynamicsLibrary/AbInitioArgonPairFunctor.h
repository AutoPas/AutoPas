...
class AbInitioArgonPairFunctor
 ...
 public:
  ...
  std::string getName() final { return "AbInitioArgonPairFunctor"; }
  ...
  inline void AoSFunctor(Particle &i, Particle &j, bool newton3) final {
    ...
    const double expAlphaTerm =
        std::exp(_alpha1 * dist + _alpha2 * dist2 + _alphaneg1 * distInv + _alphaneg2 * distInv2);
    ...
    if (calculateGlobals) {
      double potentialEnergy =
          _A * expAlphaTerm - _cConstants[0] * distInv6 * (1. - expbr * ksumacc6) -
          _cConstants[1] * distNeg8 * (1. - expbr * ksumacc8) - _cConstants[2] * distNeg10 * (1. - expbr * ksumacc10) -
          _cConstants[3] * distNeg12 * (1. - expbr * ksumacc12) -
