...
class LJFunctorXSIMD
 ...
 private:
  ...
  explicit LJFunctorXSIMD(double cutoff, void * /*dummy*/)
    ...
    // AutoPasLog(INFO, "XSIMD Wrapper initialized with a register size of ({}) with architecture {}.",
    // xsimd::batch<double>::size, xsimd::best_arch::name());
  }
 ...
 private:
  ...
  inline void SoAFunctorSingleImpl(autopas::SoAView<SoAArraysType> soa) {
    ...
    if constexpr (calculateGlobals) {
      ...
      double globals[] = {xsimd::reduce_add(virialSumX), xsimd::reduce_add(virialSumY), xsimd::reduce_add(virialSumZ),
  ...
  inline void SoAKernel(const size_t j, const xsimd::batch<int64_t> &ownedStateI,
                        const int64_t *const __restrict ownedStatePtr2, const xsimd::batch<double> &x1,
                        const xsimd::batch<double> &y1, const xsimd::batch<double> &z1,
                        ...
                        const size_t *const typeID1ptr, const size_t *const typeID2ptr, xsimd::batch<double> &fxacc,
    ...
    if constexpr (calculateGlobals) {
      ...
      if constexpr (newton3) {
        xsimd::batch_bool<double> ownedMaskJ = xsimd::eq(to_float(ownedStateJ), to_float(_ownedStateOwnedMM256i));
 ...
 private:
   ...
   * @return A * B + C
   */
  inline xsimd::batch<double> wrapperFMA(const xsimd::batch<double> &factorA, const xsimd::batch<double> &factorB,
                                         const xsimd::batch<double> &summandC) {
  ...
  const xsimd::batch<int64_t> _ownedStateOwnedMM256i{static_cast<int64_t>(autopas::OwnershipState::owned)};
