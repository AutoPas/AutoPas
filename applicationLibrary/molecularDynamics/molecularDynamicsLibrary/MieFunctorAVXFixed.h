...
class MieFunctorAVXFixed
    : public autopas::Functor<Particle, MieFunctorAVXFixed<Particle, applyShift, useMixing, useNewton3,
                                                           calculateGlobals, relevantForTuning>> {
  using SoAArraysType = typename Particle::SoAArraysType;

 private:
  /**
   * Computes and sets the DoubbleAdditionChain for the exponents (mode 2)
   * going down from m and n we apply the recursive rules and save the used rules in the chain
   */
 ...
 public:
  ...
  void setParticleProperties(double epsilon, double sigmaSquared) {
    ...
    if constexpr (applyShift) {
      _shift6 = _mm256_set1_pd(ParticlePropertiesLibrary<double, size_t>::calcShiftMie(
          cepsilon, sigmaSquared, _cutoffSquaredAoS, _nexpAoS, _mexpAoS));
    } else {
...
#ifdef __AVX__
      : autopas::Functor<Particle, MieFunctorAVXFixed<Particle, applyShift, useMixing, useNewton3, calculateGlobals,
 ...
 public:
   ...
   */
  explicit MieFunctorAVXFixed(double cutoff, ParticlePropertiesLibrary<double, size_t> &particlePropertiesLibrary)
 ...
 private:
  ...
  inline void SoAKernel(const size_t j, const __m256i ownedStateI, const int64_t *const __restrict ownedStatePtr2,
    ...
    if constexpr (mode == 0) {
      ...
      if constexpr (_mexpAoS & 1)
        miem = _mm256_sqrt_pd(mie2);
      else
        miem = _one;
      if constexpr (_nexpAoS & 1)
        mien = _mm256_sqrt_pd(mie2);
      else
        mien = _one;
 ...
 private:
  ...
  __m256d _nexp{_mm256_set1_pd(12.)};
  __m256d _mexp{_mm256_set1_pd(6.)};
  __m256d _shift6 = _mm256_setzero_pd();
  __m256d _cepsilon{};
  __m256d _cnepsilon{12};
  __m256d _cmepsilon{6};
  __m256d _sigmaSquared{};

#endif
  //probably just juse mode 0 for simple for loop
  static constexpr uint8_t mode = 2;
  const double _cutoffSquaredAoS = 0;
  // we actually do not need cepsilon
