...
#include "ArgonInclude/RepusliveTermV2.h"
#include "LUT3B.h"
#include "ParentLUT.h"
...
template <class Particle, autopas::FunctorN3Modes useNewton3 = autopas::FunctorN3Modes::Both, bool useLUT = false,
          bool useLUTGlobal = false, bool calculateGlobals = false>
class ArgonFunctor
    : public autopas::TriwiseFunctor<Particle,
                                     ArgonFunctor<Particle, useNewton3, useLUT, useLUTGlobal, calculateGlobals>> {
 ...
 public:
   ...
   */
  explicit ArgonFunctor(double cutoff, LUT3B *lut = nullptr)
      : autopas::TriwiseFunctor<Particle, ArgonFunctor<Particle, useNewton3, useLUT, useLUTGlobal, calculateGlobals>>(
            cutoff),
  ...
  void AoSFunctor(Particle &i, Particle &j, Particle &k, bool newton3) {
    ...
    std::array<double, 3> forceILUT = {0, 0, 0};
    if (useLUT) {
      std::cout << "in USELUT";
      auto res = _lut->retrieveValues(*this, distSquaredIJ, distSquaredKI, distSquaredJK);
      forceILUT = res.first;
    }
    ...
    const auto forceI_repulsive = autopas::utils::ArrayMath::Argon::F_repulsive<I>(
        A, alpha, displacementIJ, displacementJK, displacementKI, cosineI, cosineJ, cosineK);
    const auto test_forceI_rep =
        autopas::utils::ArrayMath::Argon::F_RepTermBig<I>(A, alpha, distSquaredIJ, distSquaredKI, distSquaredJK);

    const auto forceI_disperisve = autopas::utils::ArrayMath::Argon::F_dispersive<I>(
        Z, beta, displacementIJ, displacementJK, displacementKI, cosineI, cosineJ, cosineK);
    ...
    if (newton3) {
      const auto forceJ_repulsive = autopas::utils::ArrayMath::Argon::F_repulsive<J>(
          A, alpha, displacementIJ, displacementJK, displacementKI, cosineI, cosineJ, cosineK);
      const auto forceJ_disperisve = autopas::utils::ArrayMath::Argon::F_dispersive<J>(
          Z, beta, displacementIJ, displacementJK, displacementKI, cosineI, cosineJ, cosineK);
    ...
    if constexpr (calculateGlobals) {
      ...
      const auto potentialEnergy_repulsive = autopas::utils::ArrayMath::Argon::U_repulsive(
          A, alpha, displacementIJ, displacementJK, displacementKI, cosineI, cosineJ, cosineK);
      const auto potentialEnergy_disperisve = autopas::utils::ArrayMath::Argon::U_dispersive(
          Z, beta, displacementIJ, displacementJK, displacementKI, cosineI, cosineJ, cosineK);
  ...
  [[nodiscard]] std::array<double, 3> getLUTValues(double distSquaredIJ, double distSquaredJK,
    ...
    // Create points for triangle at given distances
    const std::array<double, 3> i = {0, 0, 0};
    const std::array<double, 3> j = {distIJ, 0, 0};
    auto x_value = ((distSquaredIJ + distSquaredKI - distSquaredJK) / (2 * distIJ));
    auto y_value = std::sqrt(distSquaredKI - (x_value * x_value));

    const std::array<double, 3> k = {x_value, y_value, 0};

    enum IJK { I, J, K };
    const auto IJ = autopas::utils::ArrayMath::sub(j, i);
    const auto JK = autopas::utils::ArrayMath::sub(k, j);
    const auto KI = autopas::utils::ArrayMath::sub(i, k);
    const auto distSquaredIJTest = autopas::utils::ArrayMath::dot(IJ, IJ);
    const auto distSquaredJKTest = autopas::utils::ArrayMath::dot(JK, JK);
    const auto distSquaredKITest = autopas::utils::ArrayMath::dot(KI, KI);
    ...
    const auto forceI_disperisve = autopas::utils::ArrayMath::Argon::F_dispersive<I>(
        Z, beta, displacementIJ, displacementJK, displacementKI, cosineI, cosineJ, cosineK);
    const auto forceI = autopas::utils::ArrayMath::add(forceI_repulsive, forceI_disperisve);

    return forceI;
  }

  /**
   * Added for consistency, fills the LUT
   *
