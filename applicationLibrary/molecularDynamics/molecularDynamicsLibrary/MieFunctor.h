...
class MieFunctor
    : public autopas::Functor<
          Particle, MieFunctor<Particle, applyShift, useMixing, useNewton3, calculateGlobals, relevantForTuning>> {
  /**
 ...
 private:
   ...
   */
  explicit MieFunctor(double cutoff, uint16_t n_exp, uint16_t m_exp, void * /*dummy*/)
      : autopas::Functor<Particle,
                         MieFunctor<Particle, applyShift, useMixing, useNewton3, calculateGlobals, relevantForTuning>>(
            cutoff),
 ...
 public:
   ...
   * @param cutoff
   */
  explicit MieFunctor(double cutoff, uint16_t n_exp, uint16_t m_exp) : MieFunctor(cutoff, n_exp,  m_exp, nullptr) {
   ...
   */
  explicit MieFunctor(double cutoff, uint16_t n_exp, uint16_t m_exp, ParticlePropertiesLibrary<double, size_t> &particlePropertiesLibrary)
      : MieFunctor(cutoff,  n_exp,  m_exp, nullptr) {
...
#pragma omp simd reduction(+ : fxacc, fyacc, fzacc, potentialEnergySum, virialSumX, virialSumY, virialSumZ)
      for (unsigned int j = i + 1; j < soa.getNumberOfParticles(); ++j) {
        ...
        //TODO Mie
        const SoAFloatPrecision Mie2 = sigmaSquared * invdr2;
        const SoAFloatPrecision Mie6 = Mie2 * Mie2 * Mie2;
        const SoAFloatPrecision Mie12 = Mie6 * Mie6;
        const SoAFloatPrecision Mie12m6 = Mie12 - Mie6;
        const SoAFloatPrecision fac = mask ? cepsilon * (Mie12 + Mie12m6) * invdr2 : 0.;
        ...
        if (calculateGlobals) {
          ...
          const SoAFloatPrecision potentialEnergy6 = mask ? (cepsilon * Mie12m6 + shift6) : 0.;
...
#pragma omp simd reduction(+ : fxacc, fyacc, fzacc, potentialEnergySum, virialSumX, virialSumY, virialSumZ)
      for (unsigned int j = 0; j < soa2.getNumberOfParticles(); ++j) {
        ...
        const SoAFloatPrecision Mie12 = Mie6 * Mie6;
        const SoAFloatPrecision Mie12m6 = Mie12 - Mie6;
        const SoAFloatPrecision fac = mask ? cepsilon * (Mie12 + Mie12m6) * invdr2 * mask : 0.;
        ...
        if constexpr (calculateGlobals) {
          ...
          SoAFloatPrecision potentialEnergy6 = (cepsilon * Mie12m6 + shift6) * mask;
...
#pragma omp simd reduction(+ : fxacc, fyacc, fzacc, potentialEnergySum, virialSumX, virialSumY, virialSumZ) safelen(vecsize)
        for (size_t j = 0; j < vecsize; j++) {
          ...
          if (calculateGlobals) {
            ...
            SoAFloatPrecision virialy = dry * fy;
            SoAFloatPrecision virialz = drz * fz;
            SoAFloatPrecision potentialEnergy6 = mask ? (cepsilon * Mie12m6 + shift6) : 0.;
...
#pragma omp simd safelen(vecsize)
    ...
    for (size_t jNeighIndex = joff; jNeighIndex < neighborListSize; ++jNeighIndex) {
      ...
      const SoAFloatPrecision Mie12 = Mie6 * Mie6;
      const SoAFloatPrecision Mie12m6 = Mie12 - Mie6;
      const SoAFloatPrecision fac = cepsilon * (Mie12 + Mie12m6) * invdr2;
      ...
      if (calculateGlobals) {
        ...
        SoAFloatPrecision virialy = dry * fy;
        SoAFloatPrecision virialz = drz * fz;
        SoAFloatPrecision potentialEnergy6 = (cepsilon * Mie12m6 + shift6);
  ...
  const uint16_t _nexp;
  const uint16_t _mexp;
