 ...
 * This Version is implemented using AVX512 intrinsics using gather & scatter instructions to not add force contributions from pairs of molecules beyond the cutoff.
...
class LJFunctorAVX512_GS
    : public autopas::Functor<
          Particle, LJFunctorAVX512_GS<Particle, applyShift, useMixing, useNewton3, calculateGlobals, relevantForTuning>> {
 ...
 private:
   ...
   */
  explicit LJFunctorAVX512_GS(double cutoff, void * /*dummy*/)
#ifdef __AVX512F__
      : autopas::Functor<
            Particle, LJFunctorAVX512_GS<Particle, applyShift, useMixing, useNewton3, calculateGlobals, relevantForTuning>>(
            cutoff),
    ...
    if (calculateGlobals) {
      ...
      : autopas::Functor<
            Particle, LJFunctorAVX512_GS<Particle, applyShift, useMixing, useNewton3, calculateGlobals, relevantForTuning>>(
            cutoff) {
    autopas::utils::ExceptionHandler::exception("LJFunctorAVX512_GS was compiled without AVX512 support!");
  }
#endif
 public:
  /**
   ...
   * @param cutoff
   */
  explicit LJFunctorAVX512_GS(double cutoff) : LJFunctorAVX512_GS(cutoff, nullptr) {
//    static_assert(not useMixing,
//                  "Mixing without a ParticlePropertiesLibrary is not possible! Use a different constructor or set "
//                  "mixing to false.");
    static_assert("LJFunctorAVX512_GS requires a PPL");
  }
   ...
   */
  explicit LJFunctorAVX512_GS(double cutoff, ParticlePropertiesLibrary<double, size_t> &particlePropertiesLibrary)
      : LJFunctorAVX512_GS(cutoff, nullptr) {
 ...
 private:
  ...
  inline void SoAFunctorSingleImpl(autopas::SoAView<SoAArraysType> soa) {
    ...
    for (size_t i = soa.size() - 1; (long)i >= 0; --i) {
      ...
      // Loop over molecules in SIMD. If molecule is within distance store index in array of length 16 along with distanceSquared and displacement.
      // Keep track of how many molecules "caught". If over 8, then load those 8 and run the Kernel.
      // Afterwards copy from 2nd 8 into 1st and reset counter.

      // Downside: Scatter + Later Load. Can we keep this data in data register?

      // Loop over molecules. Get mask if cutoff crit met. Compress indices into vector register.
      // Count number of molecules within cutoff criterion (1).
      // If (1) + count of number of registers already filled < 8
      //    Then update count of registers already filled.
      //    Shift registers to make space for new indices
      //    Compress idices into vector register
      // Else
      //    Compress indices into new vector register
      //    Use a mask to blend / permute the two lists together to get filled register of indices -> use move?
      //    Kernel
      ...
      for (; j < (i & ~(vecLength - 1)); j += vecLength) {
        const __m512d loopIndices = _mm512_add_epi64(_mm512_set1_epi64(j), _accendingIndices);


        const __m512d x2 = _mm512_loadu_pd(&xptr[j]);
        const __m512d y2 = _mm512_loadu_pd(&yptr[j]);
        const __m512d z2 = _mm512_loadu_pd(&zptr[j]);
        ...
        const __m512i ownershipState2 = _mm512_loadu_epi64(&ownedStatePtr[j]);
...
//    }
 ...
 private:
  ...
  __m512i _accendingIndices{_mm512_set_epi64(0,1,2,3,4,5,6,7)}; // Todo maybe remove
