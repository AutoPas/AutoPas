        ...
        void AoSFunctor(Particle &i, Particle &j, Particle &k, bool newton3) override {
          ...
          newton3 = allowsNewton3();
            ...
            if(useLUT){
                ...
                std::array<double, 4> factors  = {0,0,0};
                std::array<u_int8_t, 3> order = {0,0,0};

              std::pair<const std::array<double, 4>, std::array<u_int8_t, 3>> res = _lut->retrieveValues_global(*this, distSquaredIJ, distSquaredJK, distSquaredKI);
...
//
        ...
        [[nodiscard]] std::array<double, 4> getLUTValues(double distSquaredIJ,   double distSquaredJK, double distSquaredKI ) const {
           ...
           const auto cosines = (3. / 8.) * numerator / allDistsSquared;
            ...
            double epot =     (1.0 + cosines) * (_nu / allDistsTripled + expTerm * sum);
