...
#pragma omp simd reduction(+ : fxacc, fyacc, fzacc, potentialEnergySum, virialSumX, virialSumY, virialSumZ, numDistanceCalculationSum, numKernelCallsN3Sum, numKernelCallsNoN3Sum, numGlobalCalcsSum)
      for (unsigned int j = i + 1; j < soa.size(); ++j) {
        ...
        if (calculateGlobals) {
          ...
          SoAFloatPrecision energyFactor = (ownedStateI == 1 ? 1. : 0.) +
                                           (ownedStateJ == 1 ? 1. : 0.);
...
#pragma omp simd reduction(+ : fxacc, fyacc, fzacc, potentialEnergySum, virialSumX, virialSumY, virialSumZ, numDistanceCalculationSum, numKernelCallsN3Sum, numKernelCallsNoN3Sum, numGlobalCalcsN3Sum, numGlobalCalcsNoN3Sum)
 ...
 public:
  ...
  void SoAFunctorVerletPreloadMixingLJ(autopas::VerletListLJData<typename Particle_T::ParticleSoAFloatPrecision> &verletListLJData, const size_t indexFirst,
                      const std::vector<size_t, autopas::AlignedAllocator<size_t>> &neighborList,
                      bool newton3) {
    if (verletListLJData._forcesXXZ.size() == 0 or neighborList.empty()) return;
    if (newton3) {
      SoAFunctorVerletPreloadMixingLJPtrImpl<true>(verletListLJData, indexFirst, neighborList);
    } else {
      SoAFunctorVerletPreloadMixingLJPtrImpl<false>(verletListLJData, indexFirst, neighborList);
    }
  }
 ...
 private:
  ...
  void SoAFunctorVerletImpl(autopas::SoAView<SoAArraysType> soa, const size_t indexFirst,
    ...
    if (neighborListSize >= vecsize) {
      ...
      alignas(64) std::array<uint8_t, vecsize> ownedStateArr{};
...
#pragma omp simd safelen(vecsize)
  ...
  /*template <bool newton3>
  void SoAFunctorVerletPreloadMixingLJPtrImpl1(autopas::SoAView<SoAArraysType> soa, const size_t indexFirst,
                            const std::vector<size_t, autopas::AlignedAllocator<size_t>> &neighborList) {
  ...
  */
  ...
  template <bool newton3>
  void SoAFunctorVerletPreloadMixingLJPtrImpl(autopas::VerletListLJData<typename Particle_T::ParticleSoAFloatPrecision> &verletListLJData, const size_t indexFirst,
                            const std::vector<size_t, autopas::AlignedAllocator<size_t>> &neighborList) {

    const size_t neighborListSize = neighborList.size();
    const size_t *const __restrict neighborListPtr = neighborList.data();

    [[maybe_unused]] auto *const __restrict typeIdPtr = verletListLJData._typeIds.data();
    const auto *const __restrict ownedStatePtr = verletListLJData._ownershipStates.data();
    const auto *const __restrict positionsPtr = verletListLJData._positionsXYZ.data();
    auto * __restrict forcesPtr = verletListLJData._forcesXXZ.data();

    const SoAFloatPrecision xI = positionsPtr[indexFirst][0];
    const SoAFloatPrecision yI = positionsPtr[indexFirst][1];
    const SoAFloatPrecision zI = positionsPtr[indexFirst][2];

    const uint16_t typeIdI = typeIdPtr[indexFirst];
    ...
    using PackedEpsilon24Sigma = ParticlePropertiesLibrary<SoAFloatPrecision, unsigned long>::PackedEpsilon24Sigma;
    size_t numRegisteredSiteTypes = 0;
    const PackedEpsilon24Sigma * __restrict computedEpsilon24SigmaRow = nullptr;

    if constexpr (useMixing) {
      const auto &computedEpsilon24SigmaData = _PPLibrary->getComputedEpsilon24Sigma();
      numRegisteredSiteTypes = _PPLibrary->getNumberRegisteredSiteTypes();
      computedEpsilon24SigmaRow = computedEpsilon24SigmaData.data() + typeIdI * numRegisteredSiteTypes;
    }
...
#pragma omp simd safelen(vecSize)
        for (size_t tmpJ = 0; tmpJ < vecSize; tmpJ++) {
          const size_t liveIdJ = neighborListPtr[jOff + tmpJ];
          xArr[tmpJ] = positionsPtr[liveIdJ][0];
          yArr[tmpJ] = positionsPtr[liveIdJ][1];
          zArr[tmpJ] = positionsPtr[liveIdJ][2];
          ownedStateArr[tmpJ] = ownedStatePtr[liveIdJ];
          const uint16_t typeIdJ = typeIdPtr[liveIdJ];

          if constexpr (useMixing) {
            sigmaSquareds[tmpJ] = computedEpsilon24SigmaRow[typeIdJ].sigmaSquared;
            epsilon24s[tmpJ] = computedEpsilon24SigmaRow[typeIdJ].epsilon24;
            if constexpr (applyShift) {
              shift6s[tmpJ] = _PPLibrary->getMixingShift6(typeIdI, typeIdJ);
            }
          }
        }
        ...
        #pragma omp simd safelen(vecSize)
          for (size_t tmpj = 0; tmpj < vecSize; tmpj++) {
            const size_t j = neighborListPtr[jOff + tmpj];
            forcesPtr[j][0] -= forceXArr[tmpj];
            forcesPtr[j][1] -= forceYArr[tmpj];
            forcesPtr[j][2] -= forceZArr[tmpj];
          }
        }
      }
    }
    ...
    for (size_t jNeighIndex = jOff; jNeighIndex < neighborListSize; ++jNeighIndex) {
      ...
      const SoAFloatPrecision drX = xI - positionsPtr[j][0];
      const SoAFloatPrecision drY = yI - positionsPtr[j][1];
      const SoAFloatPrecision drZ = zI - positionsPtr[j][2];
      ...
      if constexpr (newton3) {
        forcesPtr[j][0] -= forceXIJ;
        forcesPtr[j][1] -= forceYIJ;
        forcesPtr[j][2] -= forceZIJ;
      }
    ...
    if (forceXAcc != 0 or forceYAcc != 0 or forceZAcc != 0) {
      forcesPtr[indexFirst][0] += forceXAcc;
      forcesPtr[indexFirst][1] += forceYAcc;
      forcesPtr[indexFirst][2] += forceZAcc;
    }
