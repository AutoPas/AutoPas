 ...
 * @param useLUT
 * @param useLUTGlobal uses lut and additionally saves values for potential energy in the lut
 ...
 */
    template<class Particle_T, bool applyShift = false, bool useMixing = false, bool useLUT = false, bool useLUTGlobal = false,
            autopas::FunctorN3Modes useNewton3 = autopas::FunctorN3Modes::Both, bool calculateGlobals = false,
            bool countFLOPs = false, bool relevantForTuning = true>
    class LJFunctor
            : public autopas::PairwiseFunctor<Particle_T, LJFunctor<Particle_T, applyShift, useMixing, useLUT, useLUTGlobal, useNewton3,
    ...
    private:
        ...
        explicit LJFunctor(double cutoff, void * /*dummy*/)
                : autopas::PairwiseFunctor<Particle_T, LJFunctor<Particle_T, applyShift, useMixing, useLUT, useLUTGlobal, useNewton3, calculateGlobals,
    ...
    public:
        ...
        explicit
        LJFunctor(double cutoff, LUT2B *lut = nullptr, typename std::enable_if<!useMixing, T>::type * = nullptr)
                : LJFunctor(cutoff, nullptr) {
            static_assert(not useMixing,
                          "Mixing without a ParticlePropertiesLibrary is not possible! Use a different constructor or set "
                          "mixing to false.");
            ...
            if constexpr (useLUT) {
                ...
                if (calculateGlobals) {
                    ...
                    if constexpr (useLUTGlobal) {
                        ...
                        double lj12 = lj6 * lj6;
                        lj12m6 = lj12 - lj6;
                        potentialEnergy6 = epsilon24 * lj12m6 + shift6;
                    }
                }
            } else {
                //end added for lut
                ...
                for (; joff < neighborListSize - vecsize + 1; joff += vecsize) {
                    ...
                    if constexpr (useMixing) {
                        for (size_t j = 0; j < vecsize; j++) {
                            sigmaSquareds[j] =
                                    _PPLibrary->getMixingSigmaSquared(typeptr1[indexFirst],
                                                                      typeptr2[neighborListPtr[joff + j]]);
                            epsilon24s[j] = _PPLibrary->getMixing24Epsilon(typeptr1[indexFirst],
                                                                           typeptr2[neighborListPtr[joff + j]]);
                            if constexpr (applyShift) {
                                shift6s[j] = _PPLibrary->getMixingShift6(typeptr1[indexFirst],
                                                                         typeptr2[neighborListPtr[joff + j]]);
                            }
                        }
                    }
