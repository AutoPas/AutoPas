...
#pragma omp simd reduction(+ : fxacc, fyacc, fzacc, potentialEnergySum, virialSumX, virialSumY, virialSumZ, numDistanceCalculationSum, numKernelCallsN3Sum, numKernelCallsNoN3Sum, numGlobalCalcsN3Sum, numGlobalCalcsNoN3Sum)
 ...
 public:
  ...
  void SoAFunctorVerletTryout(autopas::SoAView<SoAArraysType> soa, const size_t indexFirst,
    ...
    void SoAFunctorVerletCompactSoA(autopas::VerletListsLJCompactSoA<typename Particle_T::ParticleSoAFloatPrecision> &verletListsCompactSoAData,
                        const uint32_t indexFirst, const std::vector<uint32_t, autopas::AlignedAllocator<uint32_t>> &neighborList, bool newton3) final{
      if (verletListsCompactSoAData._ownershipStates.size() == 0 or neighborList.empty()) return;
      if (newton3) {
        SoAFunctorVerletCompactSoAImpl<true>(verletListsCompactSoAData, indexFirst, neighborList);
      } else {
        SoAFunctorVerletCompactSoAImpl<false>(verletListsCompactSoAData, indexFirst, neighborList);
      }
    }
...
#pragma omp simd safelen(vecsize)
  ...
  template <bool newton3>
  void SoAFunctorVerletImplTryout(autopas::SoAView<SoAArraysType> soa, const size_t indexFirst,
                            const std::vector<size_t, autopas::AlignedAllocator<size_t>> &neighborList) {
  ...
  class AoSThreadDataGlobals {
   ...
   private:
    ...
    double __remainingTo64[(64 - 4 * sizeof(double)) / sizeof(double)];
      ...
      template <bool newton3>
      void SoAFunctorVerletCompactSoAImpl(autopas::VerletListsLJCompactSoA<typename Particle_T::ParticleSoAFloatPrecision> &verletListsCompactSoAData, const size_t indexFirst,
                                const std::vector<uint32_t, autopas::AlignedAllocator<uint32_t>> &neighborList) {

        const auto *const __restrict xPtr = verletListsCompactSoAData._posX.data();
        const auto *const __restrict yPtr = verletListsCompactSoAData._posY.data();
        const auto *const __restrict zPtr = verletListsCompactSoAData._posZ.data();

        auto *const __restrict forceXPtr = verletListsCompactSoAData._forceX.data();
        auto *const __restrict forceYPtr = verletListsCompactSoAData._forceY.data();
        auto *const __restrict forceZPtr = verletListsCompactSoAData._forceZ.data();

        [[maybe_unused]] auto *const __restrict typeIdPtr = verletListsCompactSoAData._typeIds.data();
        const auto *const __restrict ownedStatePtr = verletListsCompactSoAData._ownershipStates.data();
        ...
        const uint8_t typeIdI = typeIdPtr[indexFirst];
        ...
        if (neighborListSize >= vecSize) {
          ...
          for (; jOff < neighborListSize - vecSize + 1; jOff += vecSize) {
            ...
            for (size_t tmpJ = 0; tmpJ < vecSize; tmpJ++) {
              ...
              if constexpr (useMixing) {
              const uint8_t typeIdJ = typeIdPtr[liveIdJ];
              const PackedLJMixingData &mixingDataJ = computedLJMixingDataRow[typeIdJ];
              sigmaSquareds[tmpJ] = mixingDataJ.sigmaSquared;
              epsilon24s[tmpJ] = mixingDataJ.epsilon24;
              if constexpr (applyShift) {
