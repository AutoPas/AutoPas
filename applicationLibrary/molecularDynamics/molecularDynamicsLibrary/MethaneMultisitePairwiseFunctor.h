...
class MethaneMultisitePairwiseFunctor
 ...
 public:
  ...
  explicit MethaneMultisitePairwiseFunctor(double cutoff) : MethaneMultisitePairwiseFunctor(cutoff, nullptr) {
    AutoPasLog(WARN, "Using MethaneMultisitePairwiseFunctor with mixing disabled is untested!");
  }
  ...
  void SoAFunctorSingle(autopas::SoAView<SoAArraysType> soa, bool newton3) final {
    ...
    for (size_t molA = 0; molA < soa.size(); ++molA) {
      const size_t noSitesInMolA = const_unrotatedSitePositions.size();  // Number of sites in molecule A
...
#pragma omp simd reduction (+ : forceSumX, forceSumY, forceSumZ, torqueSumX, torqueSumY, torqueSumZ, potentialEnergySum, virialSumX, virialSumY, virialSumZ)
        for (size_t siteB = 0; siteB < noSitesB; ++siteB) {
          ...
          // calculate forces
          const auto forceX = displacementX;
          const auto forceY = displacementY;
          const auto forceZ = displacementZ;
 ...
 private:
  ...
  void SoAFunctorPairImpl(autopas::SoAView<SoAArraysType> soaA, autopas::SoAView<SoAArraysType> soaB) {
    ...
    for (size_t molA = 0; molA < soaA.size(); ++molA) {
      ...
      const auto noSitesInMolA = const_unrotatedSitePositions.size();
      const auto unrotatedSitePositionsA = const_unrotatedSitePositions;
...
#pragma omp simd reduction (+ : forceSumX, forceSumY, forceSumZ, torqueSumX, torqueSumY, torqueSumZ, potentialEnergySum, virialSumX, virialSumY, virialSumZ)
        for (size_t siteB = 0; siteB < siteCountB; ++siteB) {
          ...
          // globals
          if constexpr (calculateGlobals) {
            const auto potentialEnergy6 = siteMask[siteB] ? (lj12m6) : 0.;
            const auto virialX = displacementX * forceX;
            const auto virialY = displacementY * forceY;
            const auto virialZ = displacementZ * forceZ;
  ...
  void SoAFunctorVerletImpl(autopas::SoAView<SoAArraysType> soa, const size_t indexPrime,
    ...
    // Count sites
    const size_t siteCountMolPrime = const_unrotatedSitePositions.size();
    ...
    const auto rotatedSitePositionsPrime = autopas::utils::quaternion::rotateVectorOfPositions(
        {q0ptr[indexPrime], q1ptr[indexPrime], q2ptr[indexPrime], q3ptr[indexPrime]}, const_unrotatedSitePositions);
...
#pragma omp simd reduction(+ : forceSumX, forceSumY, forceSumZ, torqueSumX, torqueSumY, torqueSumZ)
      for (size_t neighborSite = 0; neighborSite < siteCountNeighbors; ++neighborSite) {
        ...
        // calculate globals
        if constexpr (calculateGlobals) {
          const auto potentialEnergy6 = siteMask[neighborSite] ? (lj12m6) : 0.;
          const auto virialX = displacementX * forceX;
          const auto virialY = displacementY * forceY;
          const auto virialZ = displacementZ * forceZ;
