...
#	fi
    ...
    if [ $useGCC -eq 1 ]; then
        echo "using gcc"
	#gcc -O2 -march=pentium4 -mmmx -msse -msse2 -m32  -c -fpic OUT__1__6119__modified.c
        #gcc -O2 -march=pentium4 -mmmx -msse -msse2 -m32 -mfpmath=sse  -shared -lc -o  $out_file  OUT__1__6119__modified.o
        $GCC_COMMAND -m32 -c -fpic OUT__1__6119__modified.c
        $GCC_COMMAND -m32 -shared -lc -o  $out_file  OUT__1__6119__modified.o
	scp $out_file tiwari@spoon:~/scratch/new_code/
#	if [ $primary_or_secondary -eq 1 ]; then
#	    # this is primary conf, move this to the transport dir
#	    cp ../new_code/${out_file} $TRANSPORT_DIR
#	fi
