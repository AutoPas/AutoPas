/**
 * @file HierarchicalGridMatching.h
 * @date .02.2026
...
/**
 * HierarchicalGridMatching class.
...
class HierarchicalGridMatching : public ParticleContainerInterface<Particle> {
 public:
  /**
  ...
  /**
   * Constructor of the HierarchicalGridMatching class.
   * @param boxMin
   * @param boxMax
   ...
   */
  HierarchicalGridMatching(const std::array<double, 3> &boxMin, const std::array<double, 3> &boxMax,
    ...
    std::array<size_t, 3> cellsPerDimensionHigher = {
        static_cast<size_t>(std::llround((boxMax[0] - boxMin[0]) / tmp[0])),
        static_cast<size_t>(std::llround((boxMax[1] - boxMin[1]) / tmp[1])),
        static_cast<size_t>(std::llround((boxMax[2] - boxMin[2]) / tmp[2]))};
    ...
    for (size_t i = _numLevels - 1; i-- > 0;) {
      interactionLengths[i] = _cutoffs[i] + _skin;
      ...
      for (size_t d = 0; d < 3; ++d) {
        lowerCellsPerHigher = static_cast<size_t>(std::floor(cellLength[d] / (interactionLengths[i] * cellSizeFactor)));
        // sort out levels too close to be fitted
        if (lowerCellsPerHigher < 2) {
          _cutoffs.erase(_cutoffs.begin() + i);
      ...
      if (!_fittedGrids) {
        ...
        // level
        _levels[i] = std::make_unique<autopas::LinkedCells<Particle>>(_boxMin, _boxMax, _cutoffs.back(), skin,
    ...
    _cutoffs.shrink_to_fit();
  }

  /**
   * Destructor of HierarchicalGridMatching.
   */
  ~HierarchicalGridMatching() override = default;
   ...
   * @param obj
   */
  HierarchicalGridMatching(const HierarchicalGridMatching &obj) = delete;
   ...
   * @return
   */
  HierarchicalGridMatching &operator=(const HierarchicalGridMatching &other) = delete;
  ...
  std::string toString() const {
    ...
    text << "\n------------------------------------------\nHierarchicalGridMatching sizes: ";
  ...
  [[nodiscard]] ContainerOption getContainerType() const override { return ContainerOption::hierarchicalGridMatching; }

  [[nodiscard]] CellType getParticleCellTypeEnum() const override { return CellType::FullParticleCell; }

  /**
   * Read-only access to hierarchy levels, mainly for diagnostics and tests.
   * @return LinkedCells levels from smallest to largest cutoff.
   */
  [[nodiscard]] const std::vector<std::unique_ptr<LinkedCells<Particle>>> &getLevels() const { return _levels; }
 ...
 private:
  ...
  /**
   *
   * @param p Particle to add into HierarchicalGridMatching
  ...
  size_t getHierarchyLevel(const ParticleType &p) const {
    ...
    for (size_t i = 0; i < _numLevels; ++i) {
      // Todo: correctly appöy _cellSozeFactor
      if (_cutoffs[i] >= cutoff) {
        return i;
      }
    }
    AutoPasLog(ERROR,
               "Size of Particle({}) is bigger than biggest cutoff({}) of HierarchicalGridMatching, "
 ...
 protected:
  /**
   * Checks if a given traversal is allowed for HierarchicalGridMatching and sends required data to HGridTraversal
   * @param traversal
   */
  void prepareTraversal(TraversalInterface *traversal) {
    ...
    if (traversalInterface && hGridTraversal) {
      hGridTraversal->setLevels(&_levels, _cutoffs, _skin, this->getMaxDisplacement(), this->_stepsSinceLastRebuild,
                                _rebuildFrequency, _fittedGrids);
    } else {
      autopas::utils::ExceptionHandler::exception(
          "The selected traversal is not compatible with the HierarchicalGridMatching container. TraversalID: {}",
