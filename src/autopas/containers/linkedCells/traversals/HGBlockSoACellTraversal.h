/**
 * @file HGBlockSoACellTraversal.h
 * @author atacann
 * @date 22.04.2025
 */
...
class HGBlockSoACellTraversal : public HGTraversalBase<ParticleCell_T>, public HGTraversalInterface {
 public:
  using Particle = typename ParticleCell_T::ParticleType;

  explicit HGBlockSoACellTraversal(Functor_T *functor, DataLayoutOption dataLayout, bool useNewton3)
                  ...
                  for (size_t y = start_y; y < start_y + group[1]; ++y) {
                    for (size_t x = start_x; x < start_x + group[0]; ++x) {
                      ...
                      this->SoATraversalCellToCell(lowerLevelCB, upperLevelCB, {x, y, z}, _functor, lowerLevel,
                                                   upperLevel, interactionLengthSquared, dir, lowerBound, upperBound);
                    }
                  }
                }
              }
