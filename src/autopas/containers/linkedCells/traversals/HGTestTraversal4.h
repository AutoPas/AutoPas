...
class HGTestTraversal4 : public HGTraversalBase<ParticleCell_T>, public HGTraversalInterface {
 public:
  using Particle = typename ParticleCell_T::ParticleType;

  explicit HGTestTraversal4(Functor_T *functor, DataLayoutOption dataLayout, bool useNewton3)
            ...
            for (size_t x = start_x; x < end_x; x += stride_x) {
              for (size_t lowerLevel = 0; lowerLevel < levelLimit; lowerLevel++) {
                ...
                if (this->_dataLayout == DataLayoutOption::aos) {
                  ...
                  this->SoATraversalParticleToCellUsingOffsets(lowerLevelCB, upperLevelCB, {x, y, z}, _functor, lowerLevel,
  ...
  [[nodiscard]] TraversalOption getTraversalType() const override { return TraversalOption::hgrid_test4; };
