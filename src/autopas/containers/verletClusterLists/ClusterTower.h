...
class ClusterTower : public ParticleCell<Particle> {
 public:
  ...
  size_t generateClusters() {
    ...
    const size_t numClusters = _particlesStorage.size() / _clusterSize;
   ...
   */
  OwnershipState getPossibleParticleOwnerships() { return _particlesStorage.getPossibleParticleOwnerships(); }
