...
class VerletClusterLists : public ParticleContainerInterface<Particle_T>, public internal::ParticleDeletedObserver {
 public:
  ...
  void deleteHaloParticles() override {
    ...
    AUTOPAS_OPENMP(parallel for reduction(|| : deletedSomething) num_threads(autopas_get_tuned_num_threads()))
  ...
  [[nodiscard]] std::vector<Particle_T> updateContainer(bool keepNeighborListsValid) override {
    ...
    AUTOPAS_OPENMP(parallel reduction(vecMergeParticle
                                      : invalidParticles) num_threads(autopas_get_tuned_num_threads())) {
  ...
  [[nodiscard]] ContainerIterator<Particle_T, true, false> begin(
    ...
    if (_isValid != ValidityState::invalid) {
      ...
      return ContainerIterator<Particle_T, true, false>(*this, behavior, std::ref(additionalVectorsToPass));
    }
  }
  ...
  [[nodiscard]] ContainerIterator<Particle_T, true, true> getRegionIterator(
    ...
    if (_isValid != ValidityState::invalid) {
      ...
      appendBuffersHelper(additionalVectors, additionalVectorsToPass);
      return ContainerIterator<Particle_T, true, true>(*this, behavior, std::ref(additionalVectorsToPass), lowerCorner,
                                                       higherCorner);
    }
  }
  ...
  [[nodiscard]] ContainerIterator<Particle_T, false, true> getRegionIterator(
    ...
    if (_isValid != ValidityState::invalid) {
      ...
      appendBuffersHelper(additionalVectors, additionalVectorsToPass);
      return ContainerIterator<Particle_T, false, true>(*this, behavior, std::ref(additionalVectorsToPass), lowerCorner,
                                                        higherCorner);
    }
  }
  ...
  void traverseClustersParallel(LoopBody &&loopBody) {
    ...
    /// @todo: find sensible chunksize
    AUTOPAS_OPENMP(parallel for schedule(dynamic) collapse(2) num_threads(autopas_get_tuned_num_threads()))
