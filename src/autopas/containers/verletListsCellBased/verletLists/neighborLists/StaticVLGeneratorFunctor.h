/**
* @file StaticVLGeneratorFunctor.h
* @author Luis Gall
* @date 04.05.2023
 *
 * oriented on
...
class StaticVLGeneratorFunctor : public Functor<Particle, StaticVLGeneratorFunctor<Particle>> {
  using NeighborListsType = typename NewVerletListHelpers<Particle>::StaticNeighborListsType;
  using SoAArraysType = typename Particle::SoAArraysType;

 public:

  StaticVLGeneratorFunctor(NeighborListsType &neighborLists,
                           double interactionLength)
      : Functor<Particle, StaticVLGeneratorFunctor<Particle>>(0.),
  ...
  bool allowsNewton3() override {
    utils::ExceptionHandler::exception(
        "StaticVLGeneratorFunctor::allowsNewton3() is not implemented, because it should not be called.");
    return true;
  }

  bool allowsNonNewton3() override {
    utils::ExceptionHandler::exception(
        "StaticVLGeneratorFunctor::allowsNonNewton3() is not implemented, because it should not be called.");
    return true;
  }
  ...
  void AoSFunctor(Particle &i, Particle &j, bool /*newton3*/) override {
    ...
    if (distsquare < _interactionLengthSquared) {
      ...
      _neighborLists.at(&i).push_back(&j);
  ...
  void SoAFunctorSingle(SoAView<SoAArraysType> soa, bool newton3) override {
    ...
    for (unsigned int i = 0; i < numPart; ++i) {
      auto &currentList = _neighborLists.at(ptrptr[i]);
      ...
      for (unsigned int j = i + 1; j < numPart; ++j) {
        ...
        if (dr2 < _interactionLengthSquared) {
          ...
          if (not newton3) {
            ...
            _neighborLists.at(ptrptr[j]).push_back(ptrptr[i]);
          }
        }
      }
    }
  ...
  void SoAFunctorPair(SoAView<SoAArraysType> soa1, SoAView<SoAArraysType> soa2, bool /*newton3*/) override {
    ...
    for (unsigned int i = 0; i < numPart1; ++i) {
      auto &currentList = _neighborLists.at(ptr1ptr[i]);
