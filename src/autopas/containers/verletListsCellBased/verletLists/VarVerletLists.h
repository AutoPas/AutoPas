...
class VarVerletLists : public VerletLists<Particle> {
 public:
  ...
  VarVerletLists(const std::array<double, 3> boxMin, const std::array<double, 3> boxMax, const double cutoff,
      ...
      : VerletLists<Particle>(boxMin, boxMax, cutoff, skin, rebuildFrequency), _neighborList{} {}
  ...
  void iteratePairwise(ParticleFunctor *f, Traversal *traversal, bool useNewton3Deprecated = true) {
    if (auto *traversalInterface =
            dynamic_cast<VarVerletTraversalInterface<FullParticleCell<Particle>,
                                                     NeighborList> *>(traversal)) {
