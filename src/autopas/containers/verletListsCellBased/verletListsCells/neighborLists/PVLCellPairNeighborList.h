...
#pragma once
#include "PVLCellPairGeneratorFunctor.h"
#include "VLCCellPairGeneratorFunctor.h"
#include "PVLNeighborListInterface.h"
...
/**
 * Neighbor list to be used with PseudoVerletLists container.
 * Pseudo verlet lists iterates through each pair of neighboring cells
 * and generates a neighbor list for each particle from cell1, which consists of its, in a 1D projection, furthest potential partner from cell2.
 * @tparam Particle Type of particle to be used for this neighbor list.
 */
template <class Particle>
class PVLCellPairNeighborList : public PVLNeighborListInterface<Particle> {
 public:
  /**
   * Type of the data structure used to save the neighbor lists.
   */
  using listType = typename PseudoVerletListHelpers<Particle>::PairwiseNeighborListsType;
   ...
   */
  typename PseudoVerletListHelpers<Particle>::PairwiseNeighborListsType &getAoSNeighborList() {
  ...
  void buildAoSNeighborList(LinkedCells<Particle> &linkedCells, bool useNewton3, double cutoff, double skin,
                            ...
                            typename PseudoVerletListHelpers<Particle>::PVLBuildType::Value buildType) override {
    ...
    for (size_t firstCellIndex = 0; firstCellIndex < cellsSize; ++firstCellIndex) {
      ...
      for (auto &cellPair : _aosNeighborList[firstCellIndex]) {
        ...
        for (auto particleIter = cells[firstCellIndex].begin(); particleIter.isValid(); ++particleIter) {
          // for each particle in cell1 make pair of particle and furthest potential partner particle from cell2
  ...
  void setUpTraversal(TraversalInterface *traversal) override {
    auto vTraversal = dynamic_cast<PVLCellPairTraversalInterface<Particle> *>(traversal);

    if (vTraversal) {
      vTraversal->setPseudoVerletList(*this);
    } /**else {
      auto traversal2 = dynamic_cast<PVLCellPairTraversalInterface<Particle, PVLCellPairNeighborList<Particle>> *>(traversal);
      if (traversal2) {
        traversal2->setPseudoVerletList(*this);
      } else {
 ...
 private:
  ...
  void applyBuildFunctor(LinkedCells<Particle> &linkedCells, bool useNewton3, double cutoff, double skin,
                         double interactionLength, const TraversalOption buildTraversalOption,
                         typename PseudoVerletListHelpers<Particle>::PVLBuildType::Value buildType) override {
    PVLCellPairGeneratorFunctor<Particle> f(_aosNeighborList, _particleToCellMap, _globalToLocalIndex, cutoff + skin);
    ...
    if (buildType == PseudoVerletListHelpers<Particle>::PVLBuildType::Value::aosBuild) {
    ...
    else if (buildType == PseudoVerletListHelpers<Particle>::PVLBuildType::Value::soaBuild) {
      autopas::utils::withStaticBool(useNewton3, [&](auto n3) {
  ...
  /**
   * Internal neighbor list structure in AoS format - Pseudo Verlet lists for each particle for each cell pair.
   */
  typename PseudoVerletListHelpers<Particle>::PairwiseNeighborListsType _aosNeighborList =
