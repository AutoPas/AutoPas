...
/**
 * This functor generates pairwise pseudo verlet lists (a pseudoverlet list attached to every pair of neighboring cells).
 */
template <class Particle>

class PVLCellPairGeneratorFunctor : public Functor<Particle, PVLCellPairGeneratorFunctor<Particle>> {
  using PairwiseNeighborListsType = typename PseudoVerletListHelpers<Particle>::PairwiseNeighborListsType;
 ...
 public:
   ...
   */
  PVLCellPairGeneratorFunctor(PairwiseNeighborListsType &pseudoVerletLists,
      ...
      : Functor<Particle, PVLCellPairGeneratorFunctor<Particle>>(0.),
        _neighborLists(pseudoVerletLists),
  ...
  void AoSFunctor(Particle &i, Particle &j, bool newton3) override {
    ...
    *double distsquare = utils::ArrayMath::dot(dist, dist);
     ...
     */
      ...
      _neighborLists[cellIndex][secondCellIndexInFirst][particleIndex].second[0] = &j;
    //}
  }
 ...
 private:
  /**
   * Pairwise pseudo verlet lists: For every cell (cell1) and for each of its neighboring cells (cell2) a vector of pairs (a
   * pair for each particle). The pairs consist of a particle from cell1 and a vector of indices of its (potential) partners from
   * cell2.
   */
