...
inline void HGC08CellHandler<ParticleCell_T, PairwiseFunctor_T>::decompose2AndProcessCells(ParticleCell_T &cell1,
  ...
  for (size_t lowerLevel = 0; lowerLevel < _upperLevel; lowerLevel++) {
    ...
    if (not _fittedGrids) {
      ...
      const std::pair<std::array<double, 3>, std::array<double, 3>>[startLow, startHigh] =
          _cellBlocks[lowerLevel]->getCellBoundingBox(startIndex3D);
      std::array<double, 3> startCellCenter = 0.5 * (startHigh + startHigh);
      const std::pair<std::array<double, 3>, std::array<double, 3>>[stopLow, stopHigh] =
          _cellBlocks[lowerLevel]->getCellBoundingBox(stopIndex3D);
