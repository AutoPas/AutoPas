/**
 * @file DSSequentialTraversalMidpoint.h
 * @author F. Gratl
 * @date 11/23/18
 */
...
class DSSequentialTraversalMidpoint : public CellTraversal<ParticleCell>,
 ...
 public:
   ...
   */
  explicit DSSequentialTraversalMidpoint(Functor *functor, double cutoff, DataLayoutOption dataLayout, bool useNewton3,
...
template <class ParticleCell, class Functor>
void DSSequentialTraversalMidpoint<ParticleCell, Functor>::traverseParticles() {
  ...
  for (int i = 1; i < cells.size(); i++) {
    _cellFunctor.processCellPair(cells[0], cells[i], _boxMin, _boxMax, std::array<double, 3>(haloDirections[i]));
  }
  ...
  if constexpr (utils::isTriwiseFunctor<Functor>()) {
    ...
    for (int i = 1; i < cells.size(); i++) {
      for (int j = i + 1; j < cells.size(); j++) {
        ...
        _cellFunctor.processCellTriple(cells[0], cells[i], cells[j], _boxMin, _boxMax, haloDirections[i]);
      }
    }
  }
}
