...
class HierarchicalGrids : public ParticleContainerInterface<Particle> {
 ...
 protected:
   ...
   */
  void addParticleToGridLevel(const ParticleType &p, unsigned int level) { _hierarchyLevels[level].addParticleImpl(p); }
 ...
 public:
  ...
  void setHierarchicalGridBorders(std::vector<double> boundaries) { _hierarchicalGridBorders = boundaries; }

  std::vector<double> getHierarchicalGridBorders() { return _hierarchicalGridBorders; }
