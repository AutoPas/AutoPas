/**
 * @file VerletListsKokkosMaxNeighbors.h
 * @date 01.06.2026
 * @author Franziska Duhr
 * @note Variant of VerletListsKokkos that builds the *same* packed CSR neighbor lists (and therefore
 *       reuses VerletListsKokkosTraversalInterface / VerletListsKokkosTraversalFlat unchanged), but
 *       constructs them without the separate counting pass. If a particle exceeds maxNeighbors, maxNeighbors 
 *       is grown and the affected list is rebuilt (detect + grow & retry).
 */
...
/**
 * @class VerletListsKokkosMaxNeighbors
...
class VerletListsKokkosMaxNeighbors : public ParticleContainerInterface<Particle_T> {
    public:
    VerletListsKokkosMaxNeighbors(DataLayoutOption dataLayout, const std::array<double, 3> &boxMin, const std::array<double, 3> &boxMax, double skin, const double cutoff)
    ...
    [[nodiscard]] ContainerOption getContainerType() const override { return ContainerOption::verletListsKokkosMaxNeighbors; }
