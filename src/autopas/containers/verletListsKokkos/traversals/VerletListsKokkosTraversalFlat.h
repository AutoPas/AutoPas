...
explicit VerletListsKokkosTraversalFlat(Functor *functor, DataLayoutOption dataLayout, bool useNewton3)
    ...
    void traverseParticles() final {
      ...
      spdlog::debug("VerletListsKokkosTraversalFlat::traverseParticles: dataLayout={}, ownedParticles={}",
      ...
      if (_dataLayout == DataLayoutOption::aos) {
        ...
        spdlog::warn(
            "VerletListsKokkosTraversalFlat: AoS data layout is not supported on the CUDA backend - no forces "
            "were computed. Use SoA.");
        return;
#elif defined(KOKKOS_ENABLE_HIP)
        spdlog::warn(
            "VerletListsKokkosTraversalFlat: AoS data layout is not supported on the HIP backend - no forces "
            "were computed. Use SoA.");
        return;
#else
