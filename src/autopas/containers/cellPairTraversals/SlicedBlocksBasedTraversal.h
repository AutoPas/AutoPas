/**
 * @file SlicedBlocksBasedTraversal.h
 * @date 09 Apr 2020
 * @author henkel
 */
...
class SlicedBlocksBasedTraversal : public CellPairTraversal<ParticleCell> {
 public:
  /**
   * Constructor of the sliced block traversal.
...
#pragma omp parallel for
 ...
 protected:
  ...
  /**
   * The main traversal of the sliced block traversal.
   *
   * @copydetails C01BasedTraversal::c01Traversal()
   */
  template <typename LoopBody>
  inline void slicedBlocksTraversal(LoopBody and loopBody);
      ...
      for (unsigned long xAxis = 0; xAxis < 3; ++xAxis) {
        for (unsigned long yAxis = 0; yAxis < 3; ++yAxis) {
          for (unsigned long zAxis = 0; zAxis < 3; ++zAxis) {
            ...
            std::array<unsigned long, 3> axis = {zAxis, yAxis, xAxis};

            for (int i = 0; i < 3; ++i) {
              if (axis[i] == 0) {
                currentSubBlock[i][0] = block[0][i];
              } else if (axis[i] == 1) {
                currentSubBlock[i][0] = block[0][i] + this->_overlapAxis[0];
              } else if (axis[i] == 2) {
                currentSubBlock[i][0] = block[1][i] + 1;  // is in the next block
              }
              currentSubBlock[i][1] = axis[i];
            }
...
template <class ParticleCell, class PairwiseFunctor, DataLayoutOption::Value dataLayout, bool useNewton3>
inline void SlicedBlocksBasedTraversal<ParticleCell, PairwiseFunctor, dataLayout, useNewton3>::init(
...
template <class ParticleCell, class PairwiseFunctor, DataLayoutOption::Value dataLayout, bool useNewton3>
template <typename LoopBody>
void SlicedBlocksBasedTraversal<ParticleCell, PairwiseFunctor, dataLayout, useNewton3>::slicedBlocksTraversal(
