 ...
 */
    ...
    public:
        ...
        bool isApplicable() const override {
            const bool atLeast27Cells = _cellBlockDimensions[0].front() > 2
                                        and  _cellBlockDimensions[1].front() > 2
                                        and  _cellBlockDimensions[2].front() > 2;
...
#pragma omp parallel for
    ...
    inline void SlicedBlkBasedTraversal<ParticleCell, PairwiseFunctor, dataLayout, useNewton3>::init(
        ...
        for (int k = 0; k < 3; ++k) {
            ...
            if (rest[k] != 0) { _cellBlockDimensions[k].back() + rest[k];}
        }
        ...
        for (auto &cellBlockDimension : _cellBlockDimensions) {
            if (cellBlockDimension.front() < (unsigned long) 3 ) { return;}
            if (cellBlockDimension.back() < (unsigned long) 3 ) { return;}
        }
    ...
    void SlicedBlkBasedTraversal<ParticleCell, PairwiseFunctor, dataLayout, useNewton3>::slicedBlkTraversal(
        ...
        _cellBlocks.reserve(_cellBlockDimensions[0].size() * _cellBlockDimensions[1].size()
...
#pragma omp parallel for schedule(static, 1) num_threads(_threads)
        ...
        for (int m = 0; m < _threads; ++m) {
            ...
            for (auto &subBlock : subBlocks) {
                ...
                // >1 Cell is locked in sub_block -> goto next sub_block and put this into the queue NOT POSSIBLE because _locked = private
                // loop only over the part of the subBlock, which is actually part of the cellBlock
