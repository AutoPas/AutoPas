...
inline void CBasedTraversal<ParticleCell, PairwiseFunctor, collapseDepth>::cTraversal(
  ...
  AUTOPAS_OPENMP(parallel) {
    ...
    for (unsigned long col = 0; col < numColors; ++col) {
      ...
      if (collapseDepth == 2) {
        AUTOPAS_OPENMP(for schedule(dynamic, TraversalInterface::_openMPConfigurator.getChunkSize()) collapse(2))
        ...
        AUTOPAS_OPENMP(for schedule(dynamic, TraversalInterface::_openMPConfigurator.getChunkSize()) collapse(3))
        for (unsigned long z = start_z; z < end_z; z += stride_z) {
          for (unsigned long y = start_y; y < end_y; y += stride_y) {
            for (unsigned long x = start_x; x < end_x; x += stride_x) {
              // Don't exchange order of execution (x must be last!), it would break other code
