...
// This file is oriented on the implementation of the CellBasedParticleContainer, just that it does not use any cells to
// store particles
...
class KokkosDirectSum : public ParticleContainerInterface<Particle_T> {
 public:
  ...
  bool updateHaloParticle(const Particle_T &haloParticle) override {
    ...
    for (int i = 0; i < _haloParticles.size(); ++i) {
      ...
      if (id == haloParticle.getID()) {
        const typename Particle_T::ParticleSoAFloatPrecision x1 =
            _haloParticles.template operator()<Particle_T::AttributeNames::posX, useHostView>(i);
        const typename Particle_T::ParticleSoAFloatPrecision x2 =
            _haloParticles.template operator()<Particle_T::AttributeNames::posX, useHostView>(i);
        const typename Particle_T::ParticleSoAFloatPrecision x3 =
            _haloParticles.template operator()<Particle_T::AttributeNames::posX, useHostView>(i);
  ...
  void deleteHaloParticles() override { _haloParticles.clear(); }
  ...
  void computeInteractions(TraversalInterface *traversal) override {
    Kokkos::Profiling::pushRegion("autopas::KokkosDirectSum::computeInteractions");
  ...
  /* TODO: the current approach of massive contention locks is certainly not optimal but we can avoid costly data
     transfers to the CPU by doing everything on the GPU For the future, it might make sense to improve here if it turns
     out that also for the other containers (!), this becomes a crucial bottleneck */
  [[nodiscard]] std::vector<Particle_T> updateContainer(bool keepNeighborListsValid) override {
    ...
    if (keepNeighborListsValid) {
      // i.e.: those particles which are outside of the box shall be returned, halo particles should be marked as
      // dummies
      ...
      Kokkos::parallel_for(
          "autopas::KokkosDirectSum::collectMigrants_parallel_for",
  ...
  [[nodiscard]] ContainerIterator<Particle_T, false, false> begin(
    ...
    // (Problem: const qualifier prohibits necessary data conversions/copies...)
  ...
  // TODO: declare that changes to particles is actually undefined behavior (as no memory syncs and data conversions are
  // issued)
  template <class ExecSpace, bool regionIter, typename Result, typename Reduction, typename Lambda>
  void reduceInRegionKokkos(Lambda reduceLambda, Result &result, IteratorBehavior behavior,
                            const std::array<double, 3> &lowerCorner, const std::array<double, 3> &higherCorner,
                            const std::string &label = "reduceInRegionKokkos") {
    Kokkos::Profiling::pushRegion("autopas::KokkosDirectSum::reduceInRegion");
    // TODO: decide on how to deduce this template parameters based on the ExecSpace
    constexpr bool host = false;
    ...
    if (behavior & 0b101) {
      ...
      Kokkos::parallel_reduce(
          label + "_owned", Kokkos::RangePolicy<ExecSpace>(0, _ownedParticles.size()),
          KOKKOS_LAMBDA(int i, Result &localResult) {
            if (owned.template fulfillsIteratorRequirements<regionIter, host>(i, behavior, lowerCornerKokkos,
                                                                              higherCornerKokkos)) {
    ...
    if (behavior & 0b110) {
      ...
      auto &halo = _haloParticles;
      Kokkos::parallel_reduce(
          label + "_halo", Kokkos::RangePolicy<ExecSpace>(0, _haloParticles.size()),
          KOKKOS_LAMBDA(int i, Result &localResult) {
            if (halo.template fulfillsIteratorRequirements<regionIter, host>(i, behavior, lowerCornerKokkos,
                                                                             higherCornerKokkos)) {
 ...
 private:
  ...
  void prepareTraversal(Traversal &traversal) {
    ...
    if (kokkosDsTraversal) {
      ...
      utils::ExceptionHandler::exception(
          "The selected traversal is not compatible with the KokkosDirectSum container.");
    }
  }
  ...
  void finishTraversal(Traversal traversal) {
    ...
    if (kokkosDsTraversal) {
      ...
      if (targetLayout != _dataLayout) {
        convertTo<typename DSKokkosTraversalInterface<Particle_T>::DeviceSpace::execution_space,
                  DSKokkosTraversalInterface<Particle_T>::useHostView>(_dataLayout);
      }
    } else {
      utils::ExceptionHandler::exception(
          "The selected traversal is not compatible with the KokkosDirectSum container.");
  ...
  // Nearly a clone of DirectSum::getParticleImpl to guarantee that we fulfill all the requirements from getParticle in
  // the ParticleContainerInterface's method description
  ...
  std::tuple<const Particle_T *, size_t, size_t> getParticleImpl(size_t cellIndex, size_t particleIndex,
    ...
    const Particle_T *result = &viewToExtract.template getParticle<true>(
        particleIndex);  // TODO: make sure that view is accessible from host space
  ...
  std::tuple<size_t, size_t> advanceIteratorIndices(size_t cellIndex, size_t particleIndex,
    ...
    do {
      ...
      while (particleIndex >= (cellIndex == 0 ? _ownedParticles.size() : _haloParticles.size())) {
        ...
        ((cellIndex == 0) ? _ownedParticles.getAoS().template getParticle<true>(particleIndex)
                          : _haloParticles.getAoS().template getParticle<true>(particleIndex)),
