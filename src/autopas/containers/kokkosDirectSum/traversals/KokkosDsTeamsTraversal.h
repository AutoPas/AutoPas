 ...
 protected:
  void performTraversal(const utilsKokkos::KokkosStorage<Particle_T> &storageA,
    ...
    auto teamPolicy = Kokkos::TeamPolicy<typename DSKokkosTraversalInterface<Particle_T>::DeviceSpace::execution_space>(
        N, _teamSize, Kokkos::AUTO);

    constexpr bool calculateGlobals = Functor::globalCalculationRequested();

    if constexpr (calculateGlobals) {
      typename TeamTraversalReductionFunctor<
          Functor, Particle_T, typename DSKokkosTraversalInterface<Particle_T>::DeviceSpace>::ReductionResult
          globalResult{};
      TeamTraversalReductionFunctor<Functor, Particle_T, typename DSKokkosTraversalInterface<Particle_T>::DeviceSpace>
          reductionFunctor{storageA, storageB, func, cutoffSquared, M};
      ...
      TeamTraversalFunctor<Functor, Particle_T, typename DSKokkosTraversalInterface<Particle_T>::DeviceSpace> functor{
          storageA, storageB, func, cutoffSquared, M};
