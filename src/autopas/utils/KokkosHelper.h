...
namespace autopas{
  ...
  public:
      ...
      static KOKKOS_FLOAT dot(FloatVectorType const &a, FloatVectorType const &b){
        KOKKOS_FLOAT res = 0;
        Kokkos::parallel_reduce(KOKKOS_DIM, KOKKOS_LAMBDA ( int i, double &update ) {
          update += a(i) * b(i);
        }, res);
        return res;
      }
      ...
      KOKKOS_INLINE_FUNCTION
      static void subDotMulScalarAddF(FloatVectorType const &target, FloatVectorType const &source, FloatVectorType const &f, KOKKOS_FLOAT const &s){
        //Kokkos::parallel_for(KOKKOS_DIM, KOKKOS_LAMBDA(const int i){
        KOKKOS_FLOAT temp;
        for(int i = 0; i < KOKKOS_DIM; i++){
