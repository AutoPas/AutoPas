...
TraversalSelector<ParticleCell>::generatePairwiseTraversal(TraversalOption traversalType,
  ...
  switch (traversalType) {
    ...
    case TraversalOption::ds_sequential: {
      auto dsTraversal = std::make_unique<DSSequentialTraversal<ParticleCell, PairwiseFunctor>>(
          &pairwiseFunctor,
          traversalInfo
              .interactionLength /*this is the cutoff, as generated by DirectSum::getTraversalSelectorInfo()!*/,
          dataLayout, useNewton3);
      return std::unique_ptr<TraversalInterface>(dynamic_cast<TraversalInterface *>(dsTraversal.release()));
    }
    // Linked cell
