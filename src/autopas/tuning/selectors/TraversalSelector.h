...
std::unique_ptr<TraversalInterface> TraversalSelector::generateTriwiseTraversal(
  ...
  switch (traversalType) {
    ...
    // VerletLists
    case TraversalOption::vl_list_iteration: {
      return std::make_unique<VLListIterationTraversal<ParticleCell_T, TriwiseFunctor_T>>(&triwiseFunctor, dataLayout,
                                                                                      useNewton3);
    }
    case TraversalOption::vl_list_intersection_sorted_3b: {
      return std::make_unique<VLListIntersectionTraversalSorted3B<ParticleCell_T, TriwiseFunctor_T>>(
          &triwiseFunctor, dataLayout, useNewton3);
    }
    case TraversalOption::vl_list_intersection_hashing_3b: {
      return std::make_unique<VLListIntersectionTraversalHashing3B<ParticleCell_T, TriwiseFunctor_T>>(
          &triwiseFunctor, dataLayout, useNewton3);
    }
    case TraversalOption::vl_pair_list_iteration_3b: {
      return std::make_unique<VLPairListIterationTraversal3B<ParticleCell_T, TriwiseFunctor_T>>(&triwiseFunctor, dataLayout,
                                                                                            useNewton3);
    }
    default: {
