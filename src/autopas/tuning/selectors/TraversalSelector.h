...
TraversalSelector<ParticleCell, interactionType>::generateTriwiseTraversal(TraversalOption traversalType,
                                                                           ...
                                                                           const TraversalSelectorInfo &traversalInfo, DataLayoutOption dataLayout,
                                                                           bool useNewton3) {
  switch (traversalType) {
    // Direct sum
    case TraversalOption::ds_sequential_3b: {
      return std::make_unique<DSSequentialTraversal3B<ParticleCell, TriwiseFunctor>>(
          &triwiseFunctor,
          traversalInfo.interactionLength /*this is the cutoff, as generated by DirectSum::getTraversalSelectorInfo()!*/,
          dataLayout,
          useNewton3);
    }
      // Linked Cells
    case TraversalOption::lc_c01_3b: {
      return std::make_unique<LCC01Traversal3B<ParticleCell, TriwiseFunctor>>(
          traversalInfo.cellsPerDim, &triwiseFunctor, traversalInfo.interactionLength, traversalInfo.cellLength, dataLayout, useNewton3);
    }
    default: {
