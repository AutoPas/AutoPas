...
bool autopas::SlowConfigFilter::optimizeSuggestions(std::vector<Configuration> &configQueue,
  ...
  const auto blacklistThreshold =
      static_cast<long>(static_cast<double>(bestEvidence.effectiveValue) * _relativeBlacklistRange);
  ...
  for (const auto &conf : configQueue) {
    ...
    if (evidenceVec and not evidenceVec->empty()) {
      ...
      if (latestEvidence.tuningPhase == bestEvidence.tuningPhase and
          latestEvidence.effectiveValue > blacklistThreshold) {
