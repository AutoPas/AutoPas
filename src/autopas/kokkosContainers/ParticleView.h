...
class ParticleView {
 public:
  ...
  void forEach(Lambda forEachLambda, std::string label = "") {
    ...
    _forEach<parallel, false, false>(forEachLambda, autopas::IteratorBehavior::ownedOrHaloOrDummy, {0, _size}, dummy,
                                     dummy, label);
  }
  ...
  void forEach(Lambda forEachLambda, std::array<double, 3> lowerCorner, std::array<double, 3> higherCorner,
               std::string label = "") {
    _forEach<parallel, false, true>(forEachLambda, autopas::IteratorBehavior::ownedOrHaloOrDummy, {0, _size},
  ...
  void forEach(Lambda forEachLambda, std::array<double, 3> lowerCorner, std::array<double, 3> higherCorner,
    ...
    _forEach<parallel, true, true>(forEachLambda, behavior, {0, _size}, lowerCorner, higherCorner, label);
  }

  template <bool parallel, typename Lambda>
  void forEach(Lambda forEachLambda, autopas::KokkosParticleCell<ParticleType> cell, std::string label = "") {
    std::array<double, 3> dummy{};
    _forEach<parallel, false, false>(forEachLambda, autopas::IteratorBehavior::ownedOrHaloOrDummy, cell.getRange(),
                                     dummy, dummy, label);
  }
  ...
  void forEach(Lambda forEachLambda, autopas::IteratorBehavior behavior, autopas::KokkosParticleCell<ParticleType> cell,
    ...
    _forEach<parallel, true, true>(forEachLambda, behavior, cell.getRange(), lowerCorner, higherCorner, label);
  }
