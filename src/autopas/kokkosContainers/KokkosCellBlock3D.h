...
class KokkosCellBlock3D : public CellBorderAndFlagManager {
 public:
  /**
  ...
  /**
   * Constructor of KokkosCellBlock3D
   ...
   */
  KokkosCellBlock3D(Kokkos::View<ParticleCell *> cells, const std::array<double, 3> bMin,
                    const std::array<double, 3> bMax, double interactionLength, double cellSizeFactor = 1.0) {
    rebuild(cells, bMin, bMax, interactionLength, cellSizeFactor);
    ...
    for (int i = 0; i < 3; ++i) {
      if (bMax[i] < bMin[i] + interactionLength) {
        ...
        utils::ExceptionHandler::exception("Error in KokkosCellBlock3D: interaction Length too large!");
      }
    }
  }
   ...
   */
  KokkosCellBlock3D(const KokkosCellBlock3D &) = delete;

  /**
   * delete assignment operator
   * @return deleted
   */
  KokkosCellBlock3D &operator=(const KokkosCellBlock3D) = delete;
   ...
   * @param cellSizeFactor cell size factor relative to interactionLength
   */
  void rebuild(Kokkos::View<ParticleCell *> &cells, const std::array<double, 3> &bMin,
               const std::array<double, 3> &bMax, double interactionLength, double cellSizeFactor);
...
template <class ParticleCell>
inline typename KokkosCellBlock3D<ParticleCell>::index_t KokkosCellBlock3D<ParticleCell>::get1DIndexOfPosition(
    const std::array<double, 3> &pos) const {
  return index1D(get3DIndexOfPosition(pos));
}

template <class ParticleCell>
inline std::array<typename KokkosCellBlock3D<ParticleCell>::index_t, 3>
KokkosCellBlock3D<ParticleCell>::get3DIndexOfPosition(const std::array<double, 3> &pos) const {
  std::array<typename KokkosCellBlock3D<ParticleCell>::index_t, 3> cellIndex{};
...
template <class ParticleCell>
inline void KokkosCellBlock3D<ParticleCell>::rebuild(Kokkos::View<ParticleCell *> &cells,
...
template <class ParticleCell>
inline std::pair<std::array<double, 3>, std::array<double, 3>> KokkosCellBlock3D<ParticleCell>::getCellBoundingBox(
...
template <class ParticleCell>
inline ParticleCell &KokkosCellBlock3D<ParticleCell>::getContainingCell(const std::array<double, 3> &pos) const {
  auto ind = get1DIndexOfPosition(pos);
  return getCell(ind);
}

template <class ParticleCell>
inline ParticleCell &KokkosCellBlock3D<ParticleCell>::getCell(index_t index1d) const {
  return _cells->at(index1d);
}

template <class ParticleCell>
inline ParticleCell &KokkosCellBlock3D<ParticleCell>::getCell(const std::array<index_t, 3> &index3d) const {
  return _cells->at(index1D(index3d));
}

template <class ParticleCell>
inline std::array<typename KokkosCellBlock3D<ParticleCell>::index_t, 3> KokkosCellBlock3D<ParticleCell>::index3D(
    index_t index1d) const {
  return utils::ThreeDimensionalMapping::oneToThreeD(index1d, _cellsPerDimensionWithHalo);
}

template <class ParticleCell>
inline typename KokkosCellBlock3D<ParticleCell>::index_t KokkosCellBlock3D<ParticleCell>::index1D(
    const std::array<index_t, 3> &index3d) const {
  return utils::ThreeDimensionalMapping::threeToOneD(index3d, _cellsPerDimensionWithHalo);
}

template <class ParticleCell>
bool KokkosCellBlock3D<ParticleCell>::checkInHalo(const std::array<double, 3> &position) const {
  return autopas::utils::inBox(position, _haloBoxMin, _haloBoxMax) &&
         autopas::utils::notInBox(position, _boxMin, _boxMax);
}

template <class ParticleCell>
void KokkosCellBlock3D<ParticleCell>::clearHaloCells() {
