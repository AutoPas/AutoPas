...
class CellFunctorMidpoint {
 public:
   ...
   */
  explicit CellFunctorMidpoint(ParticleFunctor *f, const double sortingCutoff, DataLayoutOption dataLayout,
                               bool useNewton3)
...
bool CellFunctorMidpoint<ParticleCell, ParticleFunctor, bidirectional>::isMidpointInsideDomain(
    ParticleType *p1, ParticleType *p2, std::array<double, 3> boxMin, std::array<double, 3> boxMax) {
  using namespace autopas::utils::ArrayMath::literals;
  auto midpoint = (p1->getR() + p2->getR()) * 0.5;
  // check if midpoint is in box
  auto dist = midpoint - boxMin;
...
template <class ParticleCell, class ParticleFunctor, bool bidirectional>
void CellFunctorMidpoint<ParticleCell, ParticleFunctor, bidirectional>::setSortingThreshold(size_t sortingThreshold) {
  _sortingThreshold = sortingThreshold;
}

template <class ParticleCell, class ParticleFunctor, bool bidirectional>
void CellFunctorMidpoint<ParticleCell, ParticleFunctor, bidirectional>::processCell(ParticleCell &cell) {
...
template <class ParticleCell, class ParticleFunctor, bool bidirectional>
void CellFunctorMidpoint<ParticleCell, ParticleFunctor, bidirectional>::processCellPair(
...
template <class ParticleCell, class ParticleFunctor, bool bidirectional>
void CellFunctorMidpoint<ParticleCell, ParticleFunctor, bidirectional>::processCellAoS(ParticleCell &cell) {
  // helper function
...
template <class ParticleCell, class ParticleFunctor, bool bidirectional>
void CellFunctorMidpoint<ParticleCell, ParticleFunctor, bidirectional>::processCellPairAoSNoN3(
    ParticleCell &cell1, ParticleCell &cell2, std::array<double, 3> boxMin, std::array<double, 3> boxMax,
    const std::array<double, 3> &sortingDirection) {
  // helper function
  ...
  if ((cell1.size() + cell2.size() > _sortingThreshold) and (sortingDirection != std::array<double, 3>{0., 0., 0.})) {
    ...
    for (auto &[p1Projection, p1Ptr] : cell1Sorted._particles) {
      for (auto &[p2Projection, p2Ptr] : cell2Sorted._particles) {
        ...
        auto cell1Ownership = cell1Sorted.getPossibleParticleOwnerships();
        auto cell2Ownership = cell2Sorted.getPossibleParticleOwnerships();
        // if one is halo and the other is owned, check for midpoint
        if ((cell1Ownership == OwnershipState::halo && cell2Ownership == OwnershipState::owned) ||
            (cell1Ownership == OwnershipState::owned && cell2Ownership == OwnershipState::halo)) {
    ...
    for (auto &p1 : cell1) {
      for (auto &p2 : cell2) {
        auto cell1Ownership = cell1.getPossibleParticleOwnerships();
        auto cell2Ownership = cell2.getPossibleParticleOwnerships();
        if ((cell1Ownership == OwnershipState::halo && cell2Ownership == OwnershipState::owned) ||
            (cell1Ownership == OwnershipState::owned && cell2Ownership == OwnershipState::halo)) {
...
template <class ParticleCell, class ParticleFunctor, bool bidirectional>
void CellFunctorMidpoint<ParticleCell, ParticleFunctor, bidirectional>::processCellPairSoAN3(ParticleCell &cell1,
                                                                                             ParticleCell &cell2) {
  _functor->SoAFunctorPair(cell1._particleSoABuffer, cell2._particleSoABuffer, true);
}

template <class ParticleCell, class ParticleFunctor, bool bidirectional>
void CellFunctorMidpoint<ParticleCell, ParticleFunctor, bidirectional>::processCellPairSoANoN3(ParticleCell &cell1,
                                                                                               ParticleCell &cell2) {
...
template <class ParticleCell, class ParticleFunctor, bool bidirectional>
void CellFunctorMidpoint<ParticleCell, ParticleFunctor, bidirectional>::processCellSoAN3(ParticleCell &cell) {
  _functor->SoAFunctorSingle(cell._particleSoABuffer, true);
}

template <class ParticleCell, class ParticleFunctor, bool bidirectional>
void CellFunctorMidpoint<ParticleCell, ParticleFunctor, bidirectional>::processCellSoANoN3(ParticleCell &cell) {
