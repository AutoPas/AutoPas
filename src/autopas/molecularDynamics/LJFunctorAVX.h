...
// * @tparam useNewton3 Switch for the functor to support newton3 on, off or both. See FunctorN3Modes for possible
// values.
...
//      factor *= newton3 ? .5 : 1.;
//      // In case we have a non-cell-wise owned state, we have multiplied everything by two, so we divide it by 2
//      again. _aosThreadData[threadnum].virialSum[0] += globals[0] * factor; _aosThreadData[threadnum].virialSum[1] +=
//      globals[1] * factor; _aosThreadData[threadnum].virialSum[2] += globals[2] * factor;
...
//                        double *const __restrict fz2ptr, const size_t *const typeID1ptr, const size_t *const
//                        typeID2ptr,
...
//    }
//
//    const __m256d x2 = remainderIsMasked ? _mm256_maskload_pd(&x2ptr[j], _masks[rest - 1]) :
//    _mm256_loadu_pd(&x2ptr[j]); const __m256d y2 = remainderIsMasked ? _mm256_maskload_pd(&y2ptr[j], _masks[rest - 1])
//    : _mm256_loadu_pd(&y2ptr[j]); const __m256d z2 = remainderIsMasked ? _mm256_maskload_pd(&z2ptr[j], _masks[rest -
//    1]) : _mm256_loadu_pd(&z2ptr[j]);
//
...
//          remainderIsMasked ? _mm256_and_pd(upot, _mm256_and_pd(cutoffDummyMask, _mm256_castsi256_pd(_masks[rest -
//          1])))
