...
class LJFunctorAVX
 ...
 private:
  ...
  inline void SoAFunctorSingleImpl(SoAView<SoAArraysType> soa) {
    ...
    pack<double> virialSumX{0};
    pack<double> virialSumY{0};
    pack<double> virialSumZ{0};
    pack<double> upotSum{0};
    ...
    for (size_t i = soa.getNumberOfParticles() - 1; (long)i >= 0; --i) {
      ...
      // ownedStatePtr contains int64_t, so we broadcast these to make an pack<int64_t>.
      // _mm256_set1_epi64x broadcasts a 64-bit integer, we use this instruction to have 4 values!
      pack<int64_t> ownedStateI = vset1(ownedStatePtr[i]);

      pack<double> fxacc{0};
      pack<double> fyacc{0};
      pack<double> fzacc{0};

      const pack<double> x1 = vset1(&xptr[i]);
      const pack<double> y1 = vset1(&yptr[i]);
      const pack<double> z1 = vset1(&zptr[i]);
      ...
      const double sumfx = addv(fxacc);
      const double sumfy = addv(fyacc);
      const double sumfz = addv(fzacc);
  ...
  inline void SoAFunctorPairImpl(SoAView<SoAArraysType> soa1, SoAView<SoAArraysType> soa2) {
    ...
    for (unsigned int i = 0; i < soa1.getNumberOfParticles(); ++i) {
      ...
      static_assert(std::is_same_v<std::underlying_type_t<OwnershipState>, int64_t>,
                    "OwnershipStates underlying type should be int64_t!");
      // ownedStatePtr1 contains int64_t, so we broadcast these to make an pack<int64_t>.
      // _mm256_set1_epi64x broadcasts a 64-bit integer, we use this instruction to have 4 values!
      pack<int64_t> ownedStateI = vset1(ownedStatePtr1[i]);

      const pack<double> x1 = vset1(&x1ptr[i]);
      const pack<double> y1 = vset1(&y1ptr[i]);
      const pack<double> z1 = vset1(&z1ptr[i]);
  ...
  inline void SoAKernel(const size_t j, const pack<int64_t> ownedStateI, const int64_t *const __restrict ownedStatePtr2,
                        const pack<double> &x1, const pack<double> &y1, const pack<double> &z1, const double *const __restrict x2ptr,
                        ...
                        pack<double> &fxacc, pack<double> &fyacc, pack<double> &fzacc, pack<double> *virialSumX, pack<double> *virialSumY,
                        pack<double> *virialSumZ, pack<double> *upotSum, const unsigned int rest = 0) {
    pack<double> epsilon24s = _epsilon24;
    pack<double> sigmaSquares = _sigmaSquare;
    pack<double> shift6s = _shift6;
    if (useMixing) {
    ...
    const pack<double> x2 = remainderIsMasked ? _mm256_maskload_pd(&x2ptr[j], _masks[rest - 1]) : _mm256_loadu_pd(&x2ptr[j]);
    const pack<double> y2 = remainderIsMasked ? _mm256_maskload_pd(&y2ptr[j], _masks[rest - 1]) : _mm256_loadu_pd(&y2ptr[j]);
    const pack<double> z2 = remainderIsMasked ? _mm256_maskload_pd(&z2ptr[j], _masks[rest - 1]) : _mm256_loadu_pd(&z2ptr[j]);

    const pack<double> drx = _mm256_sub_pd(x1, x2);
    const pack<double> dry = _mm256_sub_pd(y1, y2);
    const pack<double> drz = _mm256_sub_pd(z1, z2);

    const pack<double> drx2 = _mm256_mul_pd(drx, drx);
    const pack<double> dry2 = _mm256_mul_pd(dry, dry);
    const pack<double> drz2 = _mm256_mul_pd(drz, drz);

    const pack<double> dr2PART = _mm256_add_pd(drx2, dry2);
    const pack<double> dr2 = _mm256_add_pd(dr2PART, drz2);
    ...
    const pack<double> cutoffMask = _mm256_cmp_pd(dr2, _cutoffsquare, _CMP_LE_OS);

    // This requires that dummy is zero (otherwise when loading using a mask the owned state will not be zero)
    const pack<int64_t> ownedStateJ = remainderIsMasked
                                    ? _mm256_castpd_si256(_mm256_maskload_pd(
                                          reinterpret_cast<double const *>(&ownedStatePtr2[j]), _masks[rest - 1]))
                                    : _mm256_loadu_si256(reinterpret_cast<const pack<int64_t> *>(&ownedStatePtr2[j]));
    // This requires that dummy is the first entry in OwnershipState!
    const pack<double> dummyMask = _mm256_cmp_pd(_mm256_castsi256_pd(ownedStateJ), _zero, _CMP_NEQ_UQ);
    const pack<double> cutoffDummyMask = _mm256_and_pd(cutoffMask, dummyMask);
    ...
    const pack<double> invdr2 = _mm256_div_pd(_one, dr2);
    const pack<double> lj2 = _mm256_mul_pd(sigmaSquares, invdr2);
    const pack<double> lj4 = _mm256_mul_pd(lj2, lj2);
    const pack<double> lj6 = _mm256_mul_pd(lj2, lj4);
    const pack<double> lj12 = _mm256_mul_pd(lj6, lj6);
    const pack<double> lj12m6 = _mm256_sub_pd(lj12, lj6);
    const pack<double> lj12m6alj12 = _mm256_add_pd(lj12m6, lj12);
    const pack<double> lj12m6alj12e = _mm256_mul_pd(lj12m6alj12, epsilon24s);
    const pack<double> fac = _mm256_mul_pd(lj12m6alj12e, invdr2);

    const pack<double> facMasked =
    ...
    const pack<double> fx = _mm256_mul_pd(drx, facMasked);
    const pack<double> fy = _mm256_mul_pd(dry, facMasked);
    const pack<double> fz = _mm256_mul_pd(drz, facMasked);
    ...
    if constexpr (calculateGlobals) {
      // Global Virial
      const pack<double> virialX = _mm256_mul_pd(fx, drx);
      const pack<double> virialY = _mm256_mul_pd(fy, dry);
      const pack<double> virialZ = _mm256_mul_pd(fz, drz);

      // Global Potential
      const pack<double> upot = wrapperFMA(epsilon24s, lj12m6, shift6s);

      const pack<double> upotMasked =
      ...
      pack<double> ownedMaskI =
          _mm256_cmp_pd(_mm256_castsi256_pd(ownedStateI), _mm256_castsi256_pd(_ownedStateOwnedMM256i), _CMP_EQ_UQ);
      pack<double> energyFactor = _mm256_blendv_pd(_zero, _one, ownedMaskI);
      if constexpr (newton3) {
        pack<double> ownedMaskJ =
 ...
 private:
  ...
  inline void SoAFunctorVerletImpl(SoAView<SoAArraysType> soa, const size_t indexFirst,
    ...
    // accumulators
    pack<double> virialSumX = _mm256_setzero_pd();
    pack<double> virialSumY = _mm256_setzero_pd();
    pack<double> virialSumZ = _mm256_setzero_pd();
    pack<double> upotSum = _mm256_setzero_pd();
    pack<double> fxacc = _mm256_setzero_pd();
    pack<double> fyacc = _mm256_setzero_pd();
    pack<double> fzacc = _mm256_setzero_pd();

    // broadcast particle 1
    const auto x1 = vset1(&xptr[indexFirst]);
    const auto y1 = vset1(&yptr[indexFirst]);
    const auto z1 = vset1(&zptr[indexFirst]);
    // ownedStatePtr contains int64_t, so we broadcast these to make an pack<int64_t>.
    // _mm256_set1_epi64x broadcasts a 64-bit integer, we use this instruction to have 4 values!
    pack<int64_t> ownedStateI = _mm256_set1_epi64x(static_cast<int64_t>(ownedStatePtr[indexFirst]));
    ...
    for (; j < (neighborList.size() & ~(vecLength - 1)); j += vecLength) {
      ...
      //      const pack<double> x2tmp = _mm256_i64gather_pd(&xptr[j], _vindex, 1);
      //      const pack<double> y2tmp = _mm256_i64gather_pd(&yptr[j], _vindex, 1);
      //      const pack<double> z2tmp = _mm256_i64gather_pd(&zptr[j], _vindex, 1);
      //      const pack<double> fx2tmp = _mm256_i64gather_pd(&fxptr[j], _vindex, 1);
      //      const pack<double> fy2tmp = _mm256_i64gather_pd(&fyptr[j], _vindex, 1);
      //      const pack<double> fz2tmp = _mm256_i64gather_pd(&fzptr[j], _vindex, 1);
      //      const pack<int64_t> typeID2tmp = _mm256_i64gather_epi64(&typeIDptr[j], _vindex, 1);
    ...
    if (rest > 0) {
      ...
      //      const pack<double> fy2tmp = _mm256_i64gather_pd(&fyptr[j], _vindex, 1);
      //      const pack<double> fz2tmp = _mm256_i64gather_pd(&fzptr[j], _vindex, 1);
      //      const pack<double> typeID2tmp = _mm256_i64gather_pd(&typeIDptr[j], _vindex, 1);
    ...
    // horizontally reduce fDacc to sumfD
    const pack<double> hSumfxfy = _mm256_hadd_pd(fxacc, fyacc);
    const pack<double> hSumfz = _mm256_hadd_pd(fzacc, fzacc);
    ...
    if constexpr (calculateGlobals) {
      ...
      const pack<double> hSumVirialxy = _mm256_hadd_pd(virialSumX, virialSumY);
      ...
      const pack<double> hSumVirialzUpot = _mm256_hadd_pd(virialSumZ, upotSum);
 ...
 private:
  constexpr static void initConstants(int vector_len) {
 ...
 private:
   ...
   * @return A * B + C
   */
  inline pack<double> wrapperFMA(const pack<double> &factorA, const pack<double> &factorB, const pack<double> &summandC) {
#ifdef __FMA__
    return _mm256_fmadd_pd(factorA, factorB, summandC);
#elif __AVX__
    const pack<double> tmp = _mm256_mul_pd(factorA, factorB);
  ...
  int vecLength = len<double>();
  // make sure of the size of AoSThreadData
  static_assert(sizeof(AoSThreadData) % 64 == 0, "AoSThreadData has wrong size");

  const pack<double> _zero{0};
  const pack<double> _one{1};
  const packl<double> _masks {

  };
  const pack<int64_t> _ownedStateDummyMM256i{0x0};
  const pack<int64_t> _ownedStateOwnedMM256i{_mm256_set1_epi64x(static_cast<int64_t>(OwnershipState::owned))};
  const pack<double> _cutoffsquare{};
  pack<double> _shift6 = _mm256_setzero_pd();
  pack<double> _epsilon24{};
  pack<double> _sigmaSquare{};
