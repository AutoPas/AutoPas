...
private:
 ...
 [[nodiscard]] long estimateRuntimeFromSamples() const {
   ...
   const auto numIterationsNotBuilding =
       std::max(0, static_cast<int>(_tuningStrategy->getCurrentConfiguration().verletRebuildFrequency) - static_cast<int>(_samplesRebuildingNeighborLists.size()));
   const auto numIterationsBuilding = _tuningStrategy->getCurrentConfiguration().verletRebuildFrequency - numIterationsNotBuilding;
