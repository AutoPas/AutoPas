...
std::string writeTune(bool currentInvalid) {
  return toString(std::string{"tune"}, currentInvalid);
}
...
std::string writeReset(size_t iteration) {
  return toString(std::string{"reset"}, iteration);
}
...
std::string writeLiveInfo(const LiveInfo& liveInfo) {
  return toString(std::string{"liveInfo"}, liveInfo);
}
...
TuningStrategyLoggerProxy::~TuningStrategyLoggerProxy() {
  _logOut.flush();
}
...
bool TuningStrategyLoggerProxy::needsLiveInfo() const {
  return _actualTuningStrategy->needsLiveInfo();
}
...
void TuningStrategyLoggerProxy::removeN3Option(Newton3Option option) {
  _actualTuningStrategy->removeN3Option(option);
}
bool TuningStrategyLoggerProxy::searchSpaceIsTrivial() const {
  return _actualTuningStrategy->searchSpaceIsTrivial();
}

bool TuningStrategyLoggerProxy::searchSpaceIsEmpty() const {
  return _actualTuningStrategy->searchSpaceIsEmpty();
}
    ...
    if(type == "evidence") {
      ...
      while(cont) {
        ...
        try {
          ...
          //std::cerr << "Could not find data for configuration " << _tuningStrategy->getCurrentConfiguration() << std::endl;
        }
      }
