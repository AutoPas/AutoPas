...
#include "autopas/containers/directSum/traversals/DSKokkosParallelTraversal.h"
...
std::unique_ptr<TraversalInterface> TraversalSelector<ParticleCell>::generateTraversal(
  ...
  switch (traversalType) {
    ...
    case TraversalOption::ds_kokkos_parallel: {
      return std::make_unique<DSKokkosParallelTraversal<ParticleCell, PairwiseFunctor, dataLayout, useNewton3>>(
          &pairwiseFunctor,
          info.interactionLength /*this is the cutoff, as generated by DirectSum::getTraversalSelectorInfo()!*/);
    }
//    // Linked cell
