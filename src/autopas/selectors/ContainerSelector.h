...
std::unique_ptr<autopas::ParticleContainerInterface<Particle>> ContainerSelector<Particle>::generateContainer(
  ...
  switch (containerChoice) {
    ...
    case ContainerOption::verletListsCells: {
      container = std::make_unique<VerletListsCells<Particle, VerletListsCellsNeighborList<Particle>>>(
    ...
    case ContainerOption::pairwiseVerletLists: {
      container = std::make_unique<VerletListsCells<Particle, PairwiseVerletNeighborList<Particle>>>(
          _boxMin, _boxMax, _cutoff, TraversalOption::lc_c08, containerInfo.verletSkin, containerInfo.cellSizeFactor,
          containerInfo.loadEstimator);
      break;
    }
