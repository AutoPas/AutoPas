...
class KokkosAoS {
 public:
  ...
  explicit KokkosAoS() : KokkosAoS(0) {}
  ...
  KOKKOS_INLINE_FUNCTION void addParticle(size_t index, const Particle_T &p) const {
  ...
  KOKKOS_INLINE_FUNCTION constexpr auto &operator()(size_t index) const {
    if constexpr (useHostView) {
  ...
  /* Meta Data */
  size_t size() const { return view.extent(0); }

  template <bool useHostView>
  KOKKOS_INLINE_FUNCTION Particle_T &getParticle(size_t index) {
    return useHostView ? view.view_host()(index) : view.view_device()(index);
  }

  template <bool useHostView>
  KOKKOS_INLINE_FUNCTION const Particle_T &getParticle(size_t index) const {
    return useHostView ? view.view_host()(index) : view.view_device()(index);
  }
