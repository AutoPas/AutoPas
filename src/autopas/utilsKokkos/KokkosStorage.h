...
class KokkosStorage {
 public:
  ...
  void addParticle(const Particle_T &p) {
    // TODO: figure out how to deduce that... or restict to adding particles only on the host as we cannot access the
    // device view from without a kernel
    ...
    // TODO: think about allowing another ExecSpace for conversion here (but must be consistent with the rest of the
    // function!)
  ...
  Particle_T &getParticle(int i) const { return storageAoS.getParticle(i); }
  ...
  void setActiveLayout(DataLayoutOption newLayout) { _activeLayout = newLayout; }

  void setIntendedLayout(DataLayoutOption newLayout) { _intendedLayout = newLayout; }

  DataLayoutOption getActiveLayout() const { return _activeLayout; }

  DataLayoutOption getIntendedLayout() const { return _intendedLayout; }

  KOKKOS_INLINE_FUNCTION
  KokkosAoS<Particle_T> &getAoS() { return storageAoS; }

  KOKKOS_INLINE_FUNCTION
  const KokkosAoS<Particle_T> &getAoS() const { return storageAoS; }

  KOKKOS_INLINE_FUNCTION
  Particle_T::KokkosSoAArraysType &getSoA() { return storageSoA; }

  KOKKOS_INLINE_FUNCTION
  const Particle_T::KokkosSoAArraysType &getSoA() const { return storageSoA; }

  constexpr static size_t tupleSize() { return Particle_T::KokkosSoAArraysType::tupleSize(); }

  // TODO: make it very (!) clear that this function is only to be used in exceptional cases
  void overrideSize(size_t newSize) { _currentSize = newSize; }

  void overrideCapacity(size_t newCapacity) { _capacity = newCapacity; }
 ...
 private:
  ...
  KOKKOS_INLINE_FUNCTION void copyParticleImpl(int targetIndex, const KokkosStorage<Particle_T> &otherStorage,
                                               int sourceIndex, std::index_sequence<I...>) const {
    switch (_activeLayout) {
      case DataLayoutOption::aos: {
        this->storageAoS.template addParticle<useHostView>(
            targetIndex, otherStorage.getAoS().template getParticle<useHostView>(sourceIndex));
        break;
      }
      case DataLayoutOption::soa: {
        ((this->storageSoA.template operator()<I, useHostView>(targetIndex) =
              otherStorage.getSoA().template operator()<I, useHostView>(sourceIndex)),
         ...);
        break;
      }
    }
