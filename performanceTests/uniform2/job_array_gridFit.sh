#!/bin/bash
#SBATCH --job-name=Uni_2_GridFit_Job_Array
#SBATCH --nodes=1
...
# the input_generator_gridFit.py.
...
TARGET_DIR="${SCRIPT_DIR}/generated_inputs_gridFit/totalParticles_${total_particles[${SLURM_ARRAY_TASK_ID}]}/sigmaRatio_${sigma_ratio[${SLURM_ARRAY_TASK_ID}]}/countRatio_${count_ratio[${SLURM_ARRAY_TASK_ID}]}/dataLayout_${data_layout[${SLURM_ARRAY_TASK_ID}]}"
if [ ! -d "${TARGET_DIR}" ]; then
    echo "ERROR: Input directory not found: ${TARGET_DIR}" >&2
    echo "Hint: Generate inputs with input_generator_gridFit.py before submitting." >&2
    exit 2
fi
cd "${TARGET_DIR}"
