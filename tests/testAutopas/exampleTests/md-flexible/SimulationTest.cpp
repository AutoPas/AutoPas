...
double SimulationTest::distanceBetween2Points(const std::array<double, 3>& iPos,const std::array<double, 3>& jPos) {
    return std::sqrt(std::pow((iPos[0]-jPos[0]),2.0)+std::pow((iPos[1]+jPos[1]),2.0)+std::pow((iPos[2]+jPos[2]),2.0));
}

void SimulationTest::initWithTwoParticlesWithDistance(autopas::AutoPas<PrintableMolecule, autopas::FullParticleCell<PrintableMolecule>> &autopas, double distance, double cutoff){
...
TEST_F(SimulationTest,NoMovementBetweenParticles) {
        ...
        ParticlePropertiesLibrary<double, size_t> _particlePropertiesLibrary;
        _particlePropertiesLibrary.addType(0, epsilon, sigma, mass);
        auto functor = autopas::LJFunctor<PrintableMolecule, autopas::ParticleCell<PrintableMolecule>,true>(cutoff, 0.0, _particlePropertiesLibrary);
        TimeDiscretization<decltype(autopas), decltype(_particlePropertiesLibrary)> _timeDiscretization(delta_t, _particlePropertiesLibrary);
