...
class SimulationTest : public AutoPasTestBase {
 public:
  SimulationTest() : AutoPasTestBase(), _parser{std::make_shared<YamlParser>()} {}
  ...
  void VisualizeSmallSzenario(std::array<size_t, 3> particlesPerDim, double cutoff, double particleSpacing,
                              double epsilon, double sigma, double mass, int iterations, double delta_t,
                              const std::string &filename);

  static double distanceBetween2Points(const std::array<double, 3> &iPos, const std::array<double, 3> &jPos);

  static void initWithTwoParticlesWithDistance(
