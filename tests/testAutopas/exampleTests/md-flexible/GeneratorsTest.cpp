...
//Erstellt Umgebung von Blatt 2 Task 3 des MolSim Praktikums
void MolSimTaskGeneration(autopas::AutoPas<PrintableMolecule,FullParticleCell<PrintableMolecule>> &autopas){
    ...
    PrintableMolecule dummyParticle;
    //small Grid
    GridGenerator::fillWithParticlesOnRInitialV(autopas,smallGridBoxMin,initialVelocitySmall,{8,8,1},dummyParticle,{.5,.5,.5},{0.3,0.3,0.3});
    //Big grid
    GridGenerator::fillWithParticlesOnR(autopas,bigGridBoxMax,{40,10,1},dummyParticle,{.5,.5,.5},{0.3,0.3,0.3});
}
...
TEST(Generator,MolSimTask) {
    ...
    for (auto iter = autoPas->getContainer()->begin(); iter.isValid(); ++iter) {
        ...
        auto *functor = new autopas::LJFunctor<PrintableMolecule, autopas::ParticleCell<PrintableMolecule>, autopas::FunctorN3Modes::Both, true>(
                cutoff, epsilon, sigma, 0.0, boxmin, boxmax, true);
