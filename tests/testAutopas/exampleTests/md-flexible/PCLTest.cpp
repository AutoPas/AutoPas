...
TEST_F(PCLTest, Functions) {
  PCL.addType(0, epsilon, sigma, mass);
  PCL.addType(1, epsilon2, sigma2, mass);
  ...
  ASSERT_EQ(PCL.mixingSigmaSquare(p1.getTypeId(), p2.getTypeId()), mixingS(sigma, sigma2) * mixingS(sigma, sigma2));
}