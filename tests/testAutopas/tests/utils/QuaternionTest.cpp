...
std::array<double, 4> returnNormalizedQuaternion(std::array<double, 3> r, double theta) {
  ...
  const std::array<double, 4> quaternion = {cosThetaDiv2, r[0] * sinThetaDiv2, r[1] * sinThetaDiv2,
                                            r[2] * sinThetaDiv2};
...
TEST(QuaternionTest, qMulqTest) {
  ...
  const std::array<double, 4> expectedRes = {q1[0] * q2[0] - q1[1] * q2[1] - q1[2] * q2[2] - q1[3] * q2[3],
...
/**
 * Tests qMirror. Begins with a rigid body, made from a set of points about a center-of-mass and a non-identity
 * quaternion dictating the relative rotation of the points. We mirror the body in all three dimensions by mirroring the
 * rigid body, and then applying the correct rotation for a mirror by applying qMirror to the quaternion.
 *
 * If the quaternion is manipulated correctly, each point in the mirrored rigid body should have two coordinates equal
 * to those of the point's non-mirrored original and one coordinate which is equal in distance to the mirror plane as
 * the original. The two equal coordinates correspond to the two dimensions the mirror plane is in.
 *
...
TEST(QuaternionTest, qMirrorTest) {
  ...
  auto testMirroring = [&](int dimensionNormalToBoundary) {
    ...
    const auto originalRotatedPointPositions =
        utils::quaternion::rotateVectorOfPositions(quaternion, unrotatedPointPositions);
    ...
    const auto mirroredRotatedPointPositions =
        utils::quaternion::rotateVectorOfPositions(mirroredQuaternion, mirroredUnrotatedPointPositions);
