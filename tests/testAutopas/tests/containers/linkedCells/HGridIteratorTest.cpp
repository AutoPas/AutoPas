 ...
 */
TEST_P(HGridIteratorTest, emptyContainer) {
  ...
  autopas::AutoPas<HGridMockMolecule> autoPas;
  ...
  if (priorForceCalc) {
    ...
    EmptyPairwiseFunctor<HGridMockMolecule> eFunctor;
 ...
 */
TEST_P(HGridIteratorTest, findAllParticlesInsideDomain) {
  const auto [containerOption, cellSizeFactor, useRegionIterator, useConstIterator, priorForceCalc, behavior] =
      GetParam();
 ...
 * - Find the particles with iterators and compare their IDs with expectations.
 */
TEST_P(HGridIteratorTest, findAllParticlesAroundBoundaries) {
  const auto [containerOption, cellSizeFactor, useRegionIterator, useConstIterator, priorForceCalc, behavior] =
      GetParam();
 ...
 */
TEST_P(HGridIteratorTestNonConst, deleteParticles) {
  const auto [containerOption, cellSizeFactor, useRegionIterator, useConstIterator, priorForceCalc, behavior] =
      GetParam();
...
TEST_P(HGridIteratorTestNonConstOwned, addParticlesWhileIterating) {
  const auto [containerOption, cellSizeFactor, useRegionIterator, useConstIterator, priorForceCalc, behavior] =
      GetParam();
  ...
  IteratorTestHelper::provideIterator</* const iter */ false>(
      autoPas, behavior, useRegionIterator, [&](auto &autopas, auto &getIter) {
        for (auto iterator = getIter(); iterator.isValid(); ++iterator) {
          ...
          const HGridMockMolecule newParticle{newPosition, {}, iterator->getID() + idOffset};
...
INSTANTIATE_TEST_SUITE_P(Generated, HGridIteratorTest,
                         Combine(Values(autopas::ContainerOption::hierarchicalGridMatching),
