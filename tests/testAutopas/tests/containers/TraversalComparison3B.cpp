/**
 * @file TraversalComparison3B.cpp
 * @author muehlhaeusser
 * @date 15.09.23
 */

#include "TraversalComparison3B.h"
 ...
 * @return shift vector
 */
std::array<double, 3> randomShift3B(double magnitude, std::mt19937 &generator) {
...
void TraversalComparison3B::executeShift(ContainerType &container, double magnitude, size_t totalNumParticles) {
  ...
  for (auto &elem : shiftVectorByID) {
    elem = randomShift3B(magnitude, generator);
  }
...
std::tuple<std::vector<std::array<double, 3>>, TraversalComparison3B::Globals> TraversalComparison3B::calculateForces(
  ...
  auto traversal =
      autopas::utils::withStaticCellType<Molecule>(container.getParticleCellTypeEnum(), [&](auto particleCellDummy) {
        return autopas::TraversalSelector<decltype(particleCellDummy)>::template generateTraversal<decltype(functor)>(
 ...
 */
void TraversalComparison3B::generateReference(mykey_t key) {
  ...
  if (globals) {
    auto [calculatedForces, calculatedGlobals] =
        calculateForces<true>(autopas::ContainerOption::linkedCells, autopas::TraversalOption::lc_c01_3b,
                              autopas::DataLayoutOption::aos, autopas::Newton3Option::disabled, numParticles,
    ...
    auto [calculatedForces, calculatedGlobals] =
        calculateForces<false>(autopas::ContainerOption::linkedCells, autopas::TraversalOption::lc_c01_3b,
                               autopas::DataLayoutOption::aos, autopas::Newton3Option::disabled, numParticles,
                               numHaloParticles, boxMax, 1., doSlightShift, particleDeletionPosition);
    _forcesReference[key] = calculatedForces;
    _globalValuesReference[key] = calculatedGlobals;
 ...
 */
TEST_P(TraversalComparison3B, traversalTest) {
...
INSTANTIATE_TEST_SUITE_P(Generated, TraversalComparison3B, ::testing::ValuesIn(TraversalComparison3B::getTestParams()),
                         toString);
