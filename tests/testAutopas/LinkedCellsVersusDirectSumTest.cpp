		...
		_linkedCells(getBoxMin(), getBoxMax(), getCutoff())
{
	...
	autopas::LJFunctor<autopas::MoleculeLJ>::setGlobals(getCutoff(), eps, sig, shift);
}
...
void LinkedCellsVersusDirectSumTest::test(int numMolecules, double rel_err_tolerance) {
	...
	autopas::FlopCounterFunctor<autopas::MoleculeLJ> flopsDirect(getCutoff()), flopsLinked(getCutoff());
